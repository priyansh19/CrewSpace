...
Low-level reference for how CrewSpace packages are prepared and published to npm.
...
CrewSpace no longer uses release branches or Changesets for publishing.

## Why the CLI needs special packaging

The CLI package, `crewspace`, imports code from workspace packages such as:
...
### Manual first publish for `@crewspace/ui`
...
npm view @crewspace/ui version

# make sure the dist payload is fresh
pnpm --filter @crewspace/ui build
...
- If `npm view @crewspace/ui version` already returns the same version that is in [`ui/package.json`](../ui/package.json), do not republish. Bump the version or use the normal repo-wide release flow in [`scripts/release.sh`](../scripts/release.sh).
...
- For an organization-scoped package like `@crewspace/ui`, the `crewspace` npm organization must exist and the publisher must be a member with permission to publish to that scope.
...
CrewSpace uses calendar versions:
