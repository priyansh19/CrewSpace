# CrewSpace Plugin System Specification

Status: proposed complete spec for the post-V1 plugin system

This document is the complete specification for CrewSpace's plugin and extension architecture.
...
- Plugin UI bundles currently run as same-origin JavaScript inside the main CrewSpace app. Treat plugin UI as trusted code, not a sandboxed frontend capability boundary.
- Manifest capabilities currently gate worker-side host RPC calls. They do not prevent plugin UI code from calling ordinary CrewSpace HTTP APIs directly.
...
CrewSpace plugin design is based on the following assumptions:

1. CrewSpace is single-tenant and self-hosted.
2. Plugin installation is global to the instance.
3. "Companies" remain core CrewSpace business objects, but they are not plugin trust boundaries.
4. Board governance, approval gates, budget hard-stops, and core task invariants remain owned by CrewSpace core.
...
2. Let plugins add major capabilities without editing CrewSpace core.
...
4. Depend on project-local plugin folders such as `.crewspace/plugins`.
...
The single CrewSpace deployment an operator installs and controls.

### 5.2 Company

A first-class CrewSpace business object inside the instance.
...
A trusted in-process extension loaded directly by CrewSpace core.
...
An installable instance-wide extension package loaded through the CrewSpace plugin runtime.
...
CrewSpace has two extension classes.
...
CrewSpace already has a concrete workspace model:
...
Plugins live under the CrewSpace instance directory.

Suggested layout:

- `~/.crewspace/instances/default/plugins/package.json`
- `~/.crewspace/instances/default/plugins/node_modules/`
- `~/.crewspace/instances/default/plugins/.cache/`
- `~/.crewspace/instances/default/data/plugins/<plugin-id>/`
...
CrewSpace should add CLI commands:

- `pnpm crewspace plugin list`
- `pnpm crewspace plugin install <package[@version]>`
- `pnpm crewspace plugin uninstall <plugin-id>`
- `pnpm crewspace plugin upgrade <plugin-id> [version]`
- `pnpm crewspace plugin doctor <plugin-id>`
...
- UI slot IDs are automatically namespaced by plugin ID (e.g. `@crewspace/plugin-linear:sync-health-widget`), so cross-plugin collisions are structurally impossible
...
```ts
export interface CrewSpacePluginManifestV1 {
  id: string;
  apiVersion: 1;
  version: string;
  displayName: string;
  ...
  minimumCrewSpaceVersion?: string;
  capabilities: string[];
  entrypoints: {
    worker: string;
    ui?: string;
...
Plugins may contribute tools that CrewSpace agents can use during runs.
...
- CrewSpace server starts one worker process per installed plugin
...
Receives one typed CrewSpace domain event.
...
export function definePlugin(definition: PluginDefinition): CrewSpacePlugin;

/** Re-exported from Zod for config schema definitions */
export { z } from "zod";

export interface PluginContext {
  manifest: CrewSpacePluginManifestV1;
  config: {
...
```tsx
// dist/ui/index.tsx
import { usePluginData, usePluginAction, MetricCard, StatusBadge } from "@crewspace/plugin-sdk/ui";
...
- The host provides **design tokens and shared components** via `@crewspace/plugin-sdk/ui` so plugins can match the host's visual language without being forced to.
...
### 19.0.1 Plugin UI SDK (`@crewspace/plugin-sdk/ui`)
...
- Plugin bundles must not import from host internals. They may only import from `@crewspace/plugin-sdk/ui` and their own dependencies.
...
## 19.6 Shared Components In `@crewspace/plugin-sdk/ui`
...
The `@crewspace/plugin-sdk/ui` subpath should also export an `ErrorBoundary` component that plugin authors can use to catch rendering errors without crashing the host page.
...
- secret ref fields: any schema property annotated with `"format": "secret-ref"` renders as a secret picker that resolves through the CrewSpace secret provider system rather than a plain text input
...
1. Core CrewSpace data stays in first-party tables.
2. Most plugin-owned data starts in generic extension tables.
3. Plugin data should scope to existing CrewSpace objects before new tables are introduced.
4. Arbitrary third-party schema migrations are out of scope for the first plugin system.

## 21.2 Core Table Reuse

If data becomes part of the actual CrewSpace product model, it should become a first-party table.

Examples:

- `project_workspaces` is already first-party
- if CrewSpace later decides git state is core product data, it should become a first-party table too
...
2. Secret refs resolve through the existing CrewSpace secret provider system.
...
6. The operator may force-purge immediately via CLI: `pnpm crewspace plugin purge <plugin-id>`.
...
Plugin install, uninstall, upgrade, and config changes **must** take effect without restarting the CrewSpace server. This is a normative requirement, not optional.
...
### 27.1 `@crewspace/plugin-test-harness`
...
```ts
import { createTestHarness } from "@crewspace/plugin-test-harness";
...
For developing a plugin against a running CrewSpace instance:

- The operator installs the plugin from a local path: `pnpm crewspace plugin install ./path/to/plugin`
...
The host should publish a starter template (`create-crewspace-plugin`) that scaffolds:

- `package.json` with correct `crewspacePlugin` keys
...
- `@crewspace/plugin-workspace-files`
- `@crewspace/plugin-terminal`
- `@crewspace/plugin-git`
- `@crewspace/plugin-linear`
- `@crewspace/plugin-github-issues`
- `@crewspace/plugin-grafana`
- `@crewspace/plugin-runtime-processes`
- `@crewspace/plugin-stripe`
...
- `@crewspace/plugin-sdk` — the complete plugin SDK

The package uses subpath exports to separate worker and UI concerns:

- `@crewspace/plugin-sdk` — worker-side SDK (context, events, state, tools, logger, `definePlugin`, `z`)
- `@crewspace/plugin-sdk/ui` — frontend SDK (bridge hooks, shared components, design tokens)
...
2. **Tied to API version**: Each major SDK version corresponds to exactly one plugin `apiVersion`. When `@crewspace/plugin-sdk@2.x` ships, it targets `apiVersion: 2`. Plugins built with SDK 1.x continue to declare `apiVersion: 1`.
...
1. Plugin author updates `@crewspace/plugin-sdk` dependency.
...
- webhooks
- settings page
- plugin UI bundle loading, host bridge, and `@crewspace/plugin-sdk/ui`
...
- `@crewspace/plugin-test-harness`
- `create-crewspace-plugin` starter template
...
CrewSpace should not implement a generic in-process hook bag modeled directly after local coding tools.

CrewSpace should implement:
...
That is the complete target design for the CrewSpace plugin system.
