...
This guide describes the current, implemented way to create a CrewSpace plugin in this repo.
...
- Plugin UI runs as same-origin JavaScript inside the main CrewSpace app.
...
node packages/plugins/create-crewspace-plugin/dist/index.js @yourscope/plugin-name --output ./packages/plugins/examples
```

For a plugin that lives outside the CrewSpace repo:

```bash
pnpm --filter @crewspace/create-crewspace-plugin build
node packages/plugins/create-crewspace-plugin/dist/index.js @yourscope/plugin-name \
...
Inside this monorepo, the scaffold uses `workspace:*` for `@crewspace/plugin-sdk`.

Outside this monorepo, the scaffold snapshots `@crewspace/plugin-sdk` from the local CrewSpace checkout into a `.crewspace-sdk/` tarball so you can build and test a plugin without publishing anything to npm first.
...
For local development, install it into CrewSpace from an absolute local path through the plugin manager or API. The server supports local filesystem installs and watches local-path plugins for file changes so worker restarts happen automatically after rebuilds.
...
- `usePluginToast`
- `useHostContext`
- typed slot props from `@crewspace/plugin-sdk/ui`
