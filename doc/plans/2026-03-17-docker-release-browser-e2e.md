...
Today release smoke testing for published CrewSpace packages is manual and shell-driven:
...
- Docker
- `npx crewspace@canary`
- `npx crewspace@latest`
...
- runs `crewspace@${CREWSPACEAI_VERSION}` inside Docker
- waits for health
...
- email: `smoke-admin@crewspace.local`
- password: `crewspace-smoke-password`
...
- the suite passes locally against both:
  - `CREWSPACEAI_VERSION=canary`
  - `CREWSPACEAI_VERSION=latest`
...
1. A published `crewspace@canary` Docker install can be smoke-tested by Playwright in CI.
2. A published `crewspace@latest` Docker install can be smoke-tested by Playwright in CI.
