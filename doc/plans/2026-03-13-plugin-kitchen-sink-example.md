...
Add a new first-party example plugin, `Kitchen Sink (Example)`, that demonstrates every currently implemented CrewSpace plugin API surface in one place.
...
The plugin is successful if a contributor can install it and, without reading the SDK first, discover and exercise the current plugin runtime surface area from inside CrewSpace.
...
- package: `@crewspace/plugin-kitchen-sink-example`
- plugin id: `crewspace.kitchen-sink-example` or `crewspace-kitchen-sink-example`

Recommendation: use `crewspace-kitchen-sink-example` to match current in-repo example naming style.
...
- subscribe to a core CrewSpace event such as `issue.created`
...
Recommendation: default to a CrewSpace-local endpoint or a stable public echo endpoint to avoid flaky docs.
