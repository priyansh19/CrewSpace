...
This document defines the rollout plan for adapter-wide skill support in CrewSpace.
...
- CrewSpace stores desired skill state consistently even when the adapter cannot fully reconcile it
- unsupported adapters degrade clearly and safely

## 2. Current Adapter Matrix

CrewSpace currently has these adapters:
...
1. Desired skills live in CrewSpace for every adapter.
...
These adapters have a stable local skills directory that CrewSpace can read and manage.
...
These adapters do not have a meaningful CrewSpace-owned persistent install state.
...
- desired skills stored in CrewSpace
...
- runtime already injects CrewSpace skills into `~/.cursor/skills`
...
- runtime already injects CrewSpace skills into `~/.gemini/skills`
...
- runtime already injects CrewSpace skills into `~/.pi/agent/skills`
...
- CrewSpace can sync desired skills into Pi’s persistent home
...
- OpenCode currently injects CrewSpace skills into `~/.claude/skills`

This is product-risky because:

- it shares state with Claude
- CrewSpace may accidentally imply the skills belong only to OpenCode when the home is shared
...
- only remove stale managed CrewSpace skills that are clearly marked as CrewSpace-managed
...
- CrewSpace stores desired skills only
...
- avoid destructive wording unless CrewSpace can prove a skill is CrewSpace-managed
...
That gets CrewSpace from “skills work for Codex and Claude” to “skills work for the whole local-adapter family,” which is the meaningful V1 milestone.
