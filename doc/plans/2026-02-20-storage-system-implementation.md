...
Add a single storage subsystem for CrewSpace that supports:
...
- API serves attachment bytes through authenticated CrewSpace endpoints.
...
- Default local path is under instance root: `~/.crewspace/instances/<instanceId>/data/storage`.
...
- `crewspace onboard` writes a valid `storage` config block by default.
- `crewspace configure --section storage` can switch between local and s3 modes.
...
- `crewspace doctor` reports actionable storage status.
