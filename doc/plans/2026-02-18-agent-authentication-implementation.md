...
- New config/env settings:
  ...
  - `CREWSPACE_AGENT_JWT_TTL_SECONDS` (default: `172800`)
  - `CREWSPACE_AGENT_JWT_ISSUER` (default: `crewspace`)
  - `CREWSPACE_AGENT_JWT_AUDIENCE` (default: `crewspace-api`)
...
1. In:
   ...
   inject `CREWSPACE_API_KEY` from context token.

- Preserve existing behavior for explicit user-defined env vars in `adapterConfig.env`:
  - if user already sets `CREWSPACE_API_KEY`, do not overwrite it.
- Continue injecting:
  - `CREWSPACE_AGENT_ID`
...
- Local adapters authenticate without manual `CREWSPACE_API_KEY` config.
