...
CrewSpace already treats budgets as a core control-plane responsibility:
...
This plan defines the concrete budgeting model CrewSpace should implement next.

## Product Goals

CrewSpace should let operators:
...
For the next implementation pass, CrewSpace should enforce these defaults:
...
CrewSpace should separate subscription-included usage from billed spend:
...
CrewSpace should have two threshold classes:
...
CrewSpace must know that a project is budget-paused and enforce it.
...
CrewSpace must also block execution before new work starts.
