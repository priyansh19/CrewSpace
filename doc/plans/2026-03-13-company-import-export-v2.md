...
This document defines the next-stage plan for CrewSpace company import/export.

The core shift is:

- move from a CrewSpace-specific JSON-first portability package toward a markdown-first package format
...
This plan is about implementation and rollout inside CrewSpace.
...
CrewSpace already has portability primitives in the repo:
...
3. a vendor-neutral base package spec for agent-company runtimes, not just CrewSpace
4. the company package model is explicitly an extension of Agent Skills
5. no future dependency on `crewspace.manifest.json`
6. implicit folder discovery by convention for the common case
7. an always-emitted `.crewspace.yaml` sidecar for high-fidelity CrewSpace-specific details
...
11. adapter-aware skill sync surfaces so CrewSpace can read, diff, enable, disable, and reconcile skills where the adapter supports it

## 3. Product Goals

### 3.1 Goals

- A user can point CrewSpace at a local folder or GitHub repo and import a company package without any registry.
...
- Export produces a clean vendor-neutral package plus a CrewSpace sidecar.
...
CrewSpace must not redefine `SKILL.md`.
...
- CrewSpace-specific fidelity lives in `.crewspace.yaml`
- CrewSpace may resolve and install `SKILL.md` packages, but it must not require a CrewSpace-only skill format
...
### 5.4 Base Package Vs CrewSpace Extension
...
- base package:
  ...
  - no CrewSpace-only runtime fields by default
- CrewSpace extension:
  - `.crewspace.yaml`
  - adapter/runtime/permissions/budget/workspace fidelity
  - emitted by CrewSpace tools as a sidecar while the base package stays readable

### 5.5 Relationship To Current V1 Manifest

`crewspace.manifest.json` is not part of the future package direction.
...
CrewSpace import/export should support these entity kinds:
...
- CrewSpace can treat “this set of agents came from team package X” as the imported-team model
...
CrewSpace should support source references in package metadata with:
...
CrewSpace should define a new adapter surface area around skills:
...
Planned CrewSpace behavior:

- if an adapter supports read, CrewSpace should show current skills in the UI
- if an adapter supports write, CrewSpace should let the user enable/disable imported skills
- if an adapter supports sync, CrewSpace should compute desired vs actual state and offer reconcile actions
...
- omit duplicated inline prompt content from `.crewspace.yaml` when `AGENTS.md` already carries the instructions
- preserve references and attribution
- emit `.crewspace.yaml` alongside the base package
...
## 10. Storage Model Inside CrewSpace
...
CrewSpace should add managed package/provenance records so imports are not anonymous one-off copies.
...
- `crewspace company export <company-id> --out <path>`
- `crewspace company import <path-or-url> --dry-run`
- `crewspace company import <path-or-url> --target existing -C <company-id>`
...
1. Should imported skill packages be stored as managed package files in CrewSpace storage, or only referenced at import time?
...
3. Should CrewSpace support direct local folder selection in the web UI, or keep that CLI-only initially?
...
This keeps CrewSpace aligned with:
