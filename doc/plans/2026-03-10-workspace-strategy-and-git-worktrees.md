...
`PAP-447` asks how CrewSpace should support worktree-driven coding workflows for local coding agents without turning that into a universal product requirement.
...
- the workflow should reuse the same CrewSpace instance and embedded database instead of creating a blank environment
- local agent auth should remain low-friction

At the same time, we do not want to hard-code "every agent uses git worktrees" into CrewSpace:

- some operators use CrewSpace to manage CrewSpace and want worktrees heavily
- other operators will not want worktrees at all
- not every adapter runs in a local git repository
- not every adapter runs on the same machine as CrewSpace
- Claude and Codex expose different built-in affordances, so CrewSpace should not overfit to one tool

## Core Product Decision

CrewSpace should model **execution workspaces**, not **worktrees**.
...
- local adapters may receive a real filesystem cwd produced by CrewSpace
...
- CrewSpace should not assume that every adapter can see or use a host filesystem path directly
...
If CrewSpace makes worktrees agent-first, it will blur:
...
- CrewSpace core resolves and records execution workspace state
...
That gives CrewSpace room to support:
...
- CrewSpace handles workspace runtime services under the hood using project/workspace policy
...
Once CrewSpace is creating isolated issue checkouts, it is implicitly touching a bigger workflow:
...
So CrewSpace should distinguish:
...
- only add adapter-specific runtime UI when an adapter truly needs settings that CrewSpace cannot infer
...
- when issue moves to done, should CrewSpace auto-commit?
- should it auto-push?
...
CrewSpace already has the right foundation for a project-first model.
...
For server-spawned local adapters, CrewSpace already injects a short-lived local JWT:
...
- Codex CLI does not present a single first-class portable CLI worktree abstraction that CrewSpace should mirror directly

Implication:

- for `codex_local`, CrewSpace should usually create/select the checkout itself and then launch Codex inside that cwd
...
- but CrewSpace should still treat that as an adapter optimization, not the canonical cross-adapter model
...
Because of that, CrewSpace should separate:
...
CrewSpace should be able to express intentions such as:
...
These are not specific to CrewSpace. They are a common property of working in a dev workspace, whether local or remote.
...
```json
{
  "workspaceStrategy": {
    ...
    "worktreeParentDir": ".crewspace/instances/default/worktrees/projects/{{project.id}}",
...
Do not model this as a CrewSpace-specific `devServer` flag.
...
CrewSpace should distinguish between:
...
CrewSpace should normalize the reported metadata without requiring every adapter to look like a host-local process.
...
In addition to the local git helper, CrewSpace should define a generic runtime service manager contract.
...
- remote sandbox adapter: create its own isolated workspace from repo/ref/branch intent and report the realized remote workspace metadata back to CrewSpace
...
For runtime services specifically, CrewSpace should eventually track normalized fields such as:
...
- runtime services are expressed generically, not as CrewSpace-only dev-server flags
...
3. Inject resolved execution workspace details into `context.crewspaceWorkspace` for local adapters and into a generic execution-workspace intent payload for adapters that need structured remote realization.
...
- same embedded DB / CrewSpace instance can be reused through env/config injection when appropriate
...
- CrewSpace can decide whether to reuse or start a fresh service deterministically
...
3. Test against CrewSpace-on-CrewSpace workflows first.
...
7. Workspace runtime services are modeled generically, not as CrewSpace-specific dev-server toggles.
