...
Agents need API keys to authenticate with CrewSpace. The current approach
...
**Trust model:** The adapter process runs on the same machine as the CrewSpace
server (or is invoked directly by it). There is no meaningful network boundary.

**Approach:** CrewSpace generates a token and passes it directly to the agent
...
`CREWSPACE_API_URL`, `CREWSPACE_AGENT_ID`, `CREWSPACE_COMPANY_ID`. We need to
add a `CREWSPACE_API_KEY` (JWT) to the set of injected env vars.
...
**Approach:** Similar to `claude setup-token` -- the developer runs a CrewSpace CLI
...
```
crewspace auth login
# Opens browser -> user confirms -> token stored at ~/.crewspace/credentials
```
...
managed by the CrewSpace server itself.
...
1. A company admin (user or agent) generates an **invite URL** from CrewSpace.
...
3. The agent fetches the URL, which returns an **onboarding document**
   ...
   - The CrewSpace SKILL.md (or a link to it)
   - What information CrewSpace needs from the agent (e.g. webhook URL, adapter
...
5. CrewSpace stores the pending registration.
...
7. On approval, CrewSpace provisions the agent's credentials and sends the
   first heartbeat.

**Token format:** CrewSpace issues an API key (or JWT) upon approval, delivered
...
```json
{
  ...
  "onboarding": {
    ...
    "skillUrl": "https://app.crewspace.ing/skills/crewspace/SKILL.md",
    "requiredFields": {
      "name": "Your display name",
      "adapterType": "How CrewSpace should send you heartbeats",
...
1. Generate a CrewSpace invite link for the company.
...
4. A CrewSpace company member approves the new agent.
5. CrewSpace begins sending heartbeats to the OpenClaw webhook endpoint.
...
| Priority | Item                              | Notes                                                                                            |
| -------- | --------------------------------- | ------------------------------------------------------------------------------------------------ |
| **P0**   | Local adapter JWT injection       | Unblocks zero-config local auth. Mint a JWT per heartbeat, pass as `CREWSPACE_API_KEY`.          |
...
| **P3**   | CLI auth flow                     | `crewspace auth login` for developer-managed remote agents.                                      |
