...
This document defines the product and UI plan for skill management in CrewSpace.
...
- CrewSpace company import/export can include skills as package content
...
3. The UI must always tell the truth about what CrewSpace knows:
   - desired state in CrewSpace
...
CrewSpace should treat skills at two scopes:
...
- created locally inside CrewSpace later
...
7. “Understand what CrewSpace knows vs what the adapter knows.”
...
- importing from a `skills.sh`-compatible source should work without requiring a CrewSpace-specific package layout
...
This matters because Codex and similar adapters may already have local skills that CrewSpace did not install.

These should be clearly marked:

- external
- not managed by CrewSpace
...
- `desired_only`: CrewSpace wants it, adapter does not show it yet
- `external`: adapter has it but CrewSpace does not manage it
- `drifted`: adapter has a conflicting or unexpected version/location
- `unmanaged`: adapter does not support sync, CrewSpace only tracks desired state
...
- CrewSpace can still track desired skills
...
- CrewSpace can see adapter skills
...
- `.crewspace.yaml` may add CrewSpace-specific fidelity, but should not replace the base shortname association model
...
- `.crewspace.yaml` may add CrewSpace-specific skill fidelity later if needed, but should not be required for ordinary agent-to-skill association
...
CrewSpace should have a company-scoped skill model or managed package model representing:
...
CrewSpace should separately store:
...
5. If agent skill associations are path-based instead of shortname-based, the format will feel too technical and too CrewSpace-specific.
...
That gives CrewSpace one coherent skill story instead of forcing package management, adapter sync, and agent configuration into the same screen.
