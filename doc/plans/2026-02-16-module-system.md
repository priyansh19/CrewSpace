...
CrewSpace's module system lets you extend the control plane with new capabilities — revenue tracking, observability, notifications, dashboards — without forking core. Modules are self-contained packages that register routes, UI pages, database tables, and lifecycle hooks.
...
|---------|-----------|----------------|
| **Module** | A package that extends CrewSpace's API, UI, and data model | Yes |
...
modules/
  observability/
    crewspace.module.json     # manifest (required)
    src/
...
### Manifest (`crewspace.module.json`)
...
- **`id`**: Unique identifier, used as the npm package name suffix (`@crewspace/mod-observability`)
...
```typescript
import type { ModuleAPI } from "@crewspace/core";
...
import type { Db } from "@crewspace/db";
...
```
1. Scan modules/ directory for crewspace.module.json manifests
...
```jsonc
// crewspace.config.json (or env vars)
{
  "modules": {
...
export const modulePages = [
  {
    path: "/observability",
    label: "Observability",
    component: lazy(() => import("@crewspace/mod-observability/ui")),
  },
];
...
export const dashboardWidgets = [
  {
    ...
    label: "Token Burn Rate",
    placement: "dashboard",
    component: lazy(() => import("@crewspace/mod-observability/ui").then(m => ({ default: m.TokenBurnRateWidget }))),
  },
];
```
  ...
  "modules": [
    {
      ...
      "repo": "github:crewspace/mod-observability",
      "version": "0.1.0",
  ...
  "templates": [
    {
      ...
      "url": "https://store.crewspace.ing/templates/startup-in-a-box.json",
      "tags": ["startup", "team"]
    }
  ]
}
...
```bash
pnpm crewspace store list                    # browse available modules and templates
pnpm crewspace store install <module-id>     # install a module
pnpm crewspace store import <template-id>    # import a company template
pnpm crewspace store export                  # export current company as template
```
...
| **Multi-tenant** | Separate companies/orgs within one CrewSpace instance | `server:started` |
...
4. **Module config** — `crewspace.config.json` with per-module config, env var interpolation
...
10. **`@crewspace/module-sdk`** — TypeScript types for `ModuleAPI`, `HookEvent`, `HookHandler`, manifest schema
