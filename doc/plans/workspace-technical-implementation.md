...
This document translates [workspace-product-model-and-work-product.md](/Users/dotta/crewspace-subissues/doc/plans/workspace-product-model-and-work-product.md) into an implementation-ready engineering plan.
...
This repo already uses `CREWSPACE_DEPLOYMENT_MODE` for auth/deployment behavior (`local_trusted | authenticated`).
...
- `CREWSPACE_EXECUTION_TOPOLOGY=local|cloud|hybrid`
...
- `local`
  - CrewSpace may create host-local worktrees, processes, and paths
- `cloud`
  - CrewSpace should assume no durable host-local execution workspace management
...
- `provider text not null`
  - `crewspace | github | vercel | s3 | custom`
...
- use `CREWSPACE_EXECUTION_TOPOLOGY` as a defaulting guardrail
