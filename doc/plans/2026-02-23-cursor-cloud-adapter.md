...
This document defines the V1 design for a CrewSpace adapter that integrates with
...
4. synthesized stdout events for CrewSpace UI/CLI
...
3. **Agent callback auth to CrewSpace** uses a bootstrap exchange flow (no long-lived CrewSpace key in prompt).
4. **Webhooks are V1**, polling remains fallback.
5. **Skill delivery** is fetch-on-demand from CrewSpace endpoints, not full SKILL.md prompt injection.
...
- `crewspacePublicUrl` (optional override; else `CREWSPACE_PUBLIC_URL` env)
...
## CrewSpace Callback + Auth Flow (V1)

Cursor agents run remotely, so we cannot inject local env like `CREWSPACE_API_KEY`.

### Public URL

The adapter must resolve a callback base URL in this order:

1. `adapterConfig.crewspacePublicUrl`
2. `process.env.CREWSPACE_PUBLIC_URL`

If empty, fail `testEnvironment` and runtime execution with a clear error.

### Bootstrap Exchange

Goal: avoid putting long-lived CrewSpace credentials in prompt text.

Flow:

1. Before launch/follow-up, CrewSpace mints a one-time bootstrap token bound to:
   - `agentId`
   - `companyId`
   - `runId`
...
4. CrewSpace validates bootstrap token and returns a run-scoped bearer JWT.
5. Cursor agent uses returned bearer token for all CrewSpace API calls.
...
1. Prompt includes a compact instruction to fetch skills from CrewSpace.
2. After auth exchange, agent fetches:
   - `GET /api/skills/index`
   - `GET /api/skills/crewspace`
   - `GET /api/skills/crewspace-create-agent` when needed
...
- include webhook object when enabled:
  - `url: <crewspacePublicUrl>/api/adapters/cursor-cloud/webhooks`
...
4. Route by Cursor `agentId` to active CrewSpace run context.
...
5. `crewspacePublicUrl` present and reachable shape-valid
...
| Skill delivery | local skill dir injection | authenticated fetch from CrewSpace skill endpoints |
| CrewSpace auth | injected local run JWT env var | bootstrap token exchange -> run JWT |
...
2. Attach image payloads from CrewSpace context.
3. Add richer PR metadata surfacing in CrewSpace UI.
