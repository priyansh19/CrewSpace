...
CrewSpace needs to support two very different but equally valid ways of working:
...
Today, CrewSpace already has the beginnings of this model:
...
- making CrewSpace feel like a code review tool instead of a control plane
...
6. Seamlessly upgrade existing CrewSpace users to the new model without forcing disruptive reconfiguration.
7. Support cloud-hosted CrewSpace deployments where execution happens in remote or adapter-managed environments rather than local workers.

## Non-Goals

- Turning CrewSpace into a full code review product
...
This object must be recorded explicitly so that CrewSpace can:
...
CrewSpace should treat PRs as a type of work product linked back to:
...
CrewSpace already has users and existing project/task data. Any new model must preserve continuity.
...
### 6. Cloud-hosted CrewSpace must be a first-class deployment mode

CrewSpace cannot assume that it is running on the same machine as the code.

In cloud deployments, CrewSpace may:
...
- `Runtime service`: a process or service CrewSpace owns or tracks for a workspace
...
It also lets cloud-hosted CrewSpace deployments point at codebases and remotes without pretending the CrewSpace host has direct filesystem access.
...
Without an explicit `execution workspace` record, CrewSpace has nowhere stable to attach:
...
CrewSpace needs a single place to show:
...
- CrewSpace-owned processes should update health/status automatically
- external providers should at least store link, provider, external id, and latest known state
- cloud agents should be able to create work product records without CrewSpace owning the execution host
...
## 1. CrewSpace host and execution host must be decoupled
...
without the CrewSpace host itself running local git or local preview processes.
...
CrewSpace should not copy its product framing wholesale, but there are several good patterns worth applying.
...
This is a strong pattern for CrewSpace.
...
CrewSpace should keep workspace automation centered on a small set of hooks:
...
CrewSpace should provide the equivalent visibility in the project `Code` surface:
...
CrewSpace should treat execution workspaces as the natural home for:
...
CrewSpace should ensure that execution workspaces and work product have clean structured API surfaces, not just UI-only representation.
...
CrewSpace should continue treating cleanup policy as part of the core workspace model:
...
CrewSpace should not adopt these Worktrunk assumptions as universal product rules:

- every execution workspace is a local git worktree
- the CrewSpace host has direct shell and filesystem access
- every workflow is merge-centric
- every user wants developer-tool-level workspace detail in the main navigation

### Product implication

CrewSpace should borrow Worktrunk's good execution patterns while keeping the broader CrewSpace model:
...
This is how CrewSpace supports:
...
That keeps CrewSpace focused on coordination and visibility instead of splitting outputs across many hidden subsystems.
...
- cloud-hosted CrewSpace deployments remain first-class

That is a better fit for CrewSpace than either extreme:
