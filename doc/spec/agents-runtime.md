...
For `claude_local` and `codex_local`, CrewSpace assumes the CLI is already installed and authenticated on the host machine.
...
CrewSpace stores resumable session state per `(agent, taskKey, adapterType)`.
