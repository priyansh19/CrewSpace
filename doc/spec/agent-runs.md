...
This spec defines how CrewSpace actually runs agents while staying runtime-agnostic.
...
1. CrewSpace is adapter-agnostic. The key is a protocol, not a specific runtime.
...
6. Heartbeats run the configured adapter process, CrewSpace manages lifecycle, and on exit CrewSpace parses JSON output and updates state.
...
- `run.startedAt`
- `heartbeat.reason`
- `crewspace.skill` (shared CrewSpace skill text block)
...
2. Preferred transport is env vars (`CREWSPACE_*`) injected at runtime.
