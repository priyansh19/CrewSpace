...
ClipHub is the public registry where people share, discover, and download CrewSpace company configurations. A company template is a portable artifact containing an entire org — agents, reporting structure, adapter configs, role definitions, seed tasks — ready to spin up with one command.

---

## What It Is

ClipHub is to CrewSpace what a package registry is to a programming language. CrewSpace already supports exportable org configs (see [SPEC.md](./SPEC.md) §2). ClipHub is the public directory where those exports live.

A user builds a working company in CrewSpace — a dev shop, a marketing agency, a research lab, a content studio — exports the template, and publishes it to ClipHub. Anyone can browse, search, download, and spin up that company on their own CrewSpace instance.
...
A ClipHub package is a **company template export** — the portable artifact format defined in the CrewSpace spec. It contains:
...
**Install (fresh start):**
```
crewspace install cliphub:<publisher>/<company-slug>
```
Downloads the template and creates a new company in your local CrewSpace instance. You add your own API keys, set budgets, customize agents, and hit go.
...
From within CrewSpace, export your company as a template, then publish:

```
crewspace export --template my-company
crewspace publish cliphub my-company
```
...
ClipHub is a **separate service** from CrewSpace itself. CrewSpace is self-hosted; ClipHub is a hosted registry that CrewSpace instances talk to.
...
| **CrewSpace CLI** | `crewspace install`, `crewspace publish`, `crewspace cliphub sync` — built into CrewSpace |
| **CrewSpace UI** | "Browse ClipHub" panel in the CrewSpace web UI for discovering templates without leaving the app |

### Tech Stack

| Layer | Technology |
|---|---|
| Frontend | React + Vite (consistent with CrewSpace) |
| Backend | TypeScript + Hono (consistent with CrewSpace) |
| Database | PostgreSQL |
...
4. Run `crewspace install cliphub:acme/lean-saas-shop`
5. CrewSpace creates the company locally with all agents pre-configured
6. Set your API keys, adjust budgets, add your initial tasks
7. Hit go

### "I built something great and want to share it"

1. Build and iterate on a company in CrewSpace until it works well
2. Export: `crewspace export --template my-agency`
3. Publish: `crewspace publish cliphub my-agency`
...
3. Install just that agent: `crewspace install cliphub:acme/senior-python-eng --agent`
4. Assign it to a manager in your existing company
5. Done

---

## Relationship to CrewSpace

ClipHub is **not required** to use CrewSpace. You can build companies entirely from scratch without ever touching ClipHub. But ClipHub dramatically lowers the barrier to entry:
...
ClipHub is to CrewSpace what a package registry is to a language runtime: optional, but transformative.
...
- [ ] `crewspace install cliphub:<publisher>/<slug>` CLI command
...
- [ ] "Browse ClipHub" panel in CrewSpace web UI
- [ ] `crewspace cliphub sync` for bulk publishing
...
- Running companies on ClipHub (it's a registry, not a runtime — consistent with CrewSpace's own philosophy)
