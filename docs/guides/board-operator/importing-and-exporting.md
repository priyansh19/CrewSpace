...
CrewSpace companies can be exported to portable markdown packages and imported from local directories or GitHub repositories. This lets you share company configurations, duplicate setups, and version-control your agent teams.
...
├── tasks/
│   └── onboarding/TASK.md
└── .crewspace.yaml     # Adapter config, env inputs, routines
```
...
- **.crewspace.yaml** holds CrewSpace-specific config (adapter types, env inputs, budgets) as an optional sidecar.
...
```sh
crewspace company export <company-id> --out ./my-export
```
...
crewspace company export abc123 --out ./backup --include company,agents,projects

# Export everything including tasks and skills
crewspace company export abc123 --out ./full-export --include company,agents,projects,tasks,skills

# Export only specific skills
crewspace company export abc123 --out ./skills-only --include skills --skills review,deploy
```
...
- Adapter type and env input declarations in `.crewspace.yaml`
...
```sh
# From a local folder
crewspace company import ./my-export

# From a GitHub URL
crewspace company import https://github.com/org/repo

# From a GitHub subfolder
crewspace company import https://github.com/org/repo/tree/main/companies/acme

# From GitHub shorthand
crewspace company import org/repo
crewspace company import org/repo/companies/acme
```
...
If `--target` is not specified, CrewSpace infers it: if a `--company-id` is provided (or one exists in context), it defaults to `existing`; otherwise `new`.
...
```sh
crewspace company import org/repo --target existing --company-id abc123 --dry-run
```
...
```sh
crewspace company import org/company-templates/engineering-team \
  --target new \
...
```sh
crewspace company import ./shared-agents \
  --target existing \
  --company-id abc123 \
  --include agents \
  --collision rename
...
```sh
crewspace company import org/repo --ref v2.0.0 --dry-run
```

**Non-interactive import (CI/scripts):**

```sh
crewspace company import ./package \
  --target new \
  --yes \
  --json
```
...
CrewSpace supports several GitHub URL formats:
