...
CrewSpace supports two runtime modes with different security profiles.
...
```sh
pnpm crewspace allowed-hostname my-machine
```
...
When migrating from `local_trusted` to `authenticated`, CrewSpace emits a one-time claim URL at startup:
...
```sh
CREWSPACE_DEPLOYMENT_MODE=authenticated pnpm crewspace run
```
