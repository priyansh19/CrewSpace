...
Add first-class **documents** to CrewSpace as editable, revisioned, company-scoped text artifacts that can be linked to issues.
...
The `plan` key is conventional and reserved by CrewSpace workflow/docs.
...
Update the CrewSpace agent workflow so planning no longer edits the issue description.

Required changes:

- update `skills/crewspace/SKILL.md`
