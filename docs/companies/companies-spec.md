...
This specification is vendor-neutral. It is intended to be usable by any agent-company runtime, not only CrewSpace.
...
- vendors may attach the actual schedule / trigger / runtime fidelity in a vendor extension such as `.crewspace.yaml`
...
- CrewSpace must not require extra top-level fields for skill validity
- CrewSpace-specific extensions must live under `metadata.crewspace` or `metadata.sources`
...
For CrewSpace, the preferred fidelity extension is:
...
- issue/task CrewSpace-only metadata
...
- CrewSpace tools may emit the vendor extension by default as a sidecar while keeping the base markdown clean

Suggested CrewSpace shape:
...
Additional rules for CrewSpace exporters:
...
- omit empty and default-valued CrewSpace fields when possible
...
- CrewSpace exporters should emit `.crewspace.yaml` as a sidecar by default
...
## 19. CrewSpace Mapping

CrewSpace can map this spec to its runtime model like this:
...
- CrewSpace extension:
  - `.crewspace.yaml` -> adapter config, runtime config, env input declarations, permissions, budgets, routine triggers, and other CrewSpace-specific fidelity

Inline CrewSpace-only metadata that must live inside a shared markdown file should use:

- `metadata.crewspace`

That keeps the base format broader than CrewSpace.

This specification itself remains vendor-neutral and intended for any agent-company runtime, not only CrewSpace.

## 20. Cutover

CrewSpace should cut over to this markdown-first package model as the primary portability format.

`crewspace.manifest.json` does not need to be preserved as a compatibility requirement for the future package system.

For CrewSpace, this should be treated as a hard cutover in product direction rather than a long-lived dual-format strategy.
