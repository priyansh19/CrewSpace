...
Fires a webhook trigger from an external system. Requires a valid `Authorization` or `X-CrewSpace-Signature` + `X-CrewSpace-Timestamp` header pair matching the trigger's signing mode.
