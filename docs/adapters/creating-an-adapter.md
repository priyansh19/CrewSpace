...
Build a custom adapter to connect CrewSpace to any agent runtime.
...
2. Build environment with `buildCrewSpaceEnv(agent)` plus context vars
...
`format-event.ts` — pretty-prints stdout for `crewspace run --watch` using `picocolors`.
...
Make CrewSpace skills discoverable to your agent runtime without writing to the agent's working directory:
