# ClipHub: Marketplace for CrewSpace Team Configurations
...
> The "app store" for whole-company AI teams — pre-built CrewSpace configurations, agent blueprints, skills, and governance templates that ship real work from day one.

## 1. Vision & Positioning

**ClipHub** sells **entire team configurations** — org charts, agent roles, inter-agent workflows, governance rules, and project templates — for CrewSpace-managed companies.

| Dimension | ClipHub |
|---|---|
| Unit of sale | Team blueprint (multi-agent org) |
| Buyer | Founder / team lead spinning up an AI company |
| Install target | CrewSpace company (agents, projects, governance) |
...
A complete CrewSpace company configuration:
...
Single-agent configurations designed to plug into a CrewSpace org:
...
Portable skill files that any CrewSpace agent can use:

- Markdown skill files with instructions
- Tool configurations and shell scripts
- Compatible with CrewSpace's skill loading system
...
interface Listing {
  ...
  crewspaceVersionMin: string;     // Minimum CrewSpace version
...
interface Purchase {
  ...
  listingId: string;
  buyerUserId: string;
  buyerCompanyId: string | null;    // Target CrewSpace company
...
| `POST` | `/api/listings/:id/install` | Install to CrewSpace company |
...
Homepage → Browse marketplace → Filter by type/category
  ...
  → Select target CrewSpace company (or create new)
  → ClipHub API calls CrewSpace API to:
  ...
  → Redirect to CrewSpace dashboard with new team running
```
...
### 5.3 Creator: Export from CrewSpace → Publish

```
Running CrewSpace company → "Export as Blueprint" (CLI or UI)
  → CrewSpace exports:
...
- **Color palette**: Dark ink primary, warm sand backgrounds, accent color for CTAs (CrewSpace brand blue/purple)
...
| Install flow | One-click deploy to CrewSpace company |
...
{
  "targetCompanyId": "uuid",         // Existing CrewSpace company
...
3. For each agent in blueprint:
   - `POST /api/companies/:id/agents` (if `crewspace-create-agent` supports it, or via approval flow)
...
- **Frontend**: Next.js (React), Tailwind CSS, same UI framework as CrewSpace
- **Backend**: Node.js API (or extend CrewSpace server)
- **Database**: Postgres (can share CrewSpace's DB or separate)
- **Payments**: Stripe Connect (marketplace mode)
- **Storage**: S3/R2 for listing bundles and images
- **Auth**: Shared with CrewSpace auth (or OAuth2)

### 9.2 Integration with CrewSpace

ClipHub can be:
- **Option A**: A separate app that calls CrewSpace's API to install blueprints
- **Option B**: A built-in section of the CrewSpace UI (`/marketplace` route)

Option B is simpler for MVP — adds routes to the existing CrewSpace UI and API.
...
- [ ] Install flow: blueprint → CrewSpace company
...
- [ ] "Export from CrewSpace" CLI command
