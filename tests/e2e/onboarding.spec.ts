 ...
 * heartbeat fires. Set CREWSPACE_E2E_SKIP_LLM=false to enable LLM-dependent
 * assertions (requires a valid ANTHROPIC_API_KEY).
 */

const SKIP_LLM = process.env.CREWSPACE_E2E_SKIP_LLM !== "false";
...
test.describe("Onboarding wizard", () => {
  test("completes full wizard flow", async ({ page }) => {
    // Navigate directly to /onboarding so the OnboardingWizard auto-opens at
    // step 1 regardless of whether a company already exists in the database.
    ...
    expect(task.description).not.toContain("github.com/crewspaceai/companies");
