...
COMPANY_ID="${COMPANY_ID:-${CREWSPACE_COMPANY_ID:-}}"
...
SMOKE_IMAGE="${SMOKE_IMAGE:-crewspace-openclaw-smoke:local}"
SMOKE_CONTAINER_NAME="${SMOKE_CONTAINER_NAME:-crewspace-openclaw-smoke}"
...
Provide one of:
  CREWSPACE_AUTH_HEADER=\"Bearer <board-token>\"
  CREWSPACE_COOKIE=\"<board-session-cookie>\"
...
assert_status "200"
if ! grep -q "CrewSpace OpenClaw Gateway Onboarding" <<<"$RESPONSE_BODY"; then
...
for _ in $(seq 1 "$SMOKE_TIMEOUT_SEC"); do
  ...
  MATCH_COUNT="$(jq -r --arg agentId "$CREATED_AGENT_ID" '[.events[] | select(((.body.crewspace.agentId // "") == $agentId))] | length' <<<"$LAST_EVENTS")"
