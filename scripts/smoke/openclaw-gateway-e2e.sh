...
CREWSPACE_API_URL="${CREWSPACE_API_URL:-http://127.0.0.1:3100}"
API_BASE="${CREWSPACE_API_URL%/}/api"
...
OPENCLAW_CONFIG_DIR="${OPENCLAW_CONFIG_DIR:-${OPENCLAW_TMP_DIR}/openclaw-crewspace-smoke}"
...
CREWSPACE_API_URL_FOR_OPENCLAW="${CREWSPACE_API_URL_FOR_OPENCLAW:-http://host.docker.internal:3100}"
...
AUTH_HEADERS=()
if [[ -n "${CREWSPACE_AUTH_HEADER:-}" ]]; then
  AUTH_HEADERS+=( -H "Authorization: ${CREWSPACE_AUTH_HEADER}" )
fi
if [[ -n "${CREWSPACE_COOKIE:-}" ]]; then
  AUTH_HEADERS+=( -H "Cookie: ${CREWSPACE_COOKIE}" )
  CREWSPACE_BROWSER_ORIGIN="${CREWSPACE_BROWSER_ORIGIN:-${CREWSPACE_API_URL%/}}"
  AUTH_HEADERS+=( -H "Origin: ${CREWSPACE_BROWSER_ORIGIN}" -H "Referer: ${CREWSPACE_BROWSER_ORIGIN}/" )
fi
...
require_board_auth() {
  if [[ ${#AUTH_HEADERS[@]} -eq 0 ]]; then
    fail "board auth required. Set CREWSPACE_COOKIE or CREWSPACE_AUTH_HEADER."
  fi
...
create_and_approve_gateway_join() {
  ...
  join_payload="$(jq -nc \
    ...
    --arg crewspaceApiUrl "$CREWSPACE_API_URL_FOR_OPENCLAW" \
    ...
    '{
      ...
      agentDefaultsPayload: {
        ...
        sessionKey: "crewspace",
        timeoutSec: $timeoutSec,
        waitTimeoutMs: $waitTimeoutMs,
        crewspaceApiUrl: $crewspaceApiUrl
      }
    }')"
...
persist_claimed_key_artifacts() {
  ...
  local skill_dir="${OPENCLAW_CONFIG_DIR%/}/skills/crewspace"
  local claimed_file="${workspace_dir}/crewspace-claimed-api-key.json"
  local claimed_raw_file="${workspace_dir}/crewspace-claimed-api-key.raw.json"
  ...
  if [[ -n "$container" ]]; then
    ...
    docker cp "$claimed_file" "${container}:/home/node/.openclaw/workspace/crewspace-claimed-api-key.json" >/dev/null 2>&1 || true
    docker exec "$container" sh -lc "chmod 644 /home/node/.openclaw/workspace/crewspace-claimed-api-key.json" >/dev/null 2>&1 || true
  fi

  if [[ "$AUTO_INSTALL_SKILL" == "1" ]]; then
    api_request "GET" "/skills/crewspace"
    assert_status "200"
    {
      echo "CREWSPACE_API_URL=${CREWSPACE_API_URL_FOR_OPENCLAW}"
      echo
      printf "%s\n" "$RESPONSE_BODY"
    } > "${skill_dir}/SKILL.md"
...
inject_agent_api_key_payload_template() {
  ...
  local base_message
  base_message="Set CREWSPACE_API_KEY=${AGENT_API_KEY} in your run context before running CrewSpace heartbeat steps."
...
main() {
  ...
  wait_http_ready "${CREWSPACE_API_URL%/}/api/health" 15 || fail "CrewSpace API health endpoint not reachable"
  api_request "GET" "/health"
  assert_status "200"
  log "crewspace health deploymentMode=$(jq -r '.deploymentMode // "unknown"' <<<"$RESPONSE_BODY") exposure=$(jq -r '.deploymentExposure // "unknown"' <<<"$RESPONSE_BODY")"
