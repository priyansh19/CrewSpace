...
CREWSPACE_HOST_PORT="${CREWSPACE_HOST_PORT:-3100}"
CREWSPACE_HOST_FROM_CONTAINER="${CREWSPACE_HOST_FROM_CONTAINER:-host.docker.internal}"
...
COMPOSE_OVERRIDE="${OPENCLAW_DOCKER_DIR}/.crewspace-openclaw.override.yml"
...
detect_crewspace_base_url() {
  ...
  for candidate in "$CREWSPACE_HOST_FROM_CONTAINER" "$bridge_gateway"; do
    [[ -n "$candidate" ]] || continue
    health_url="http://${candidate}:${CREWSPACE_HOST_PORT}/api/health"
    if compose exec -T openclaw-gateway node -e "fetch('${health_url}').then((r)=>process.exit(r.ok?0:1)).catch(()=>process.exit(1))" >/dev/null 2>&1; then
      echo "http://${candidate}:${CREWSPACE_HOST_PORT}"
      return 0
    fi
  done
  return 1
...
crewspace_base_url="$(detect_crewspace_base_url || true)"
...
CrewSpace URL for OpenClaw container:
EOF
  if [[ -n "$crewspace_base_url" ]]; then
    cat <<EOF
  $crewspace_base_url
...
EOF
    ...
    cat <<EOF
  Auto-detect failed. Try: http://host.docker.internal:${CREWSPACE_HOST_PORT}
  (Do not use http://127.0.0.1:${CREWSPACE_HOST_PORT} inside the container.)
  If CrewSpace rejects the host, run on host machine:
    pnpm crewspaceai allowed-hostname host.docker.internal
  Then restart CrewSpace and re-run this script.
EOF
  fi
  cat <<EOF
