...
const DEFAULT_QUERY = "\"Co-Authored-By: CrewSpace <noreply@crewspace.ing>\"";
const DEFAULT_CACHE_FILE = path.resolve("data/crewspace-commit-metrics-cache.json");
...
const CREWSPACE_EMAIL = "noreply@crewspace.ing";
const CREWSPACE_NAME = "crewspace";
...
function printHelp() {
  console.log(`Usage: tsx scripts/crewspace-commit-metrics.ts [options]
...
function normalizeContributor(input: {
  ...
  if ((email && email === CREWSPACE_EMAIL) || (displayName && displayName.toLowerCase() === CREWSPACE_NAME)) {
    return null;
  }
...
function printSummary(summary: Summary) {
  console.log("CrewSpace commit metrics");
    ...
    while (true) {
      const response = await fetch(`${this.apiBase}${pathname}`, {
        headers: {
          ...
          "User-Agent": "crewspace-commit-metrics",
