...
base_cwd="${CREWSPACE_WORKSPACE_BASE_CWD:?CREWSPACE_WORKSPACE_BASE_CWD is required}"
worktree_cwd="${CREWSPACE_WORKSPACE_CWD:?CREWSPACE_WORKSPACE_CWD is required}"
crewspace_home="${CREWSPACE_HOME:-$HOME/.crewspace}"
crewspace_instance_id="${CREWSPACE_INSTANCE_ID:-default}"
crewspace_dir="$worktree_cwd/.crewspace"
worktree_config_path="$crewspace_dir/config.json"
worktree_env_path="$crewspace_dir/.env"
worktree_name="${CREWSPACE_WORKSPACE_BRANCH:-$(basename "$worktree_cwd")}"
...
source_config_path="${CREWSPACE_CONFIG:-}"
if [[ -z "$source_config_path" && ( -e "$base_cwd/.crewspace/config.json" || -L "$base_cwd/.crewspace/config.json" ) ]]; then
  source_config_path="$base_cwd/.crewspace/config.json"
fi
if [[ -z "$source_config_path" ]]; then
  source_config_path="$crewspace_home/instances/$crewspace_instance_id/config.json"
fi
source_env_path="$(dirname "$source_config_path")/.env"

mkdir -p "$crewspace_dir"

run_isolated_worktree_init() {
  if command -v pnpm >/dev/null 2>&1 && pnpm crewspaceai --help >/dev/null 2>&1; then
    pnpm crewspaceai worktree init --force --seed-mode minimal --name "$worktree_name" --from-config "$source_config_path"
    return 0
  fi

  if command -v crewspaceai >/dev/null 2>&1; then
    crewspaceai worktree init --force --seed-mode minimal --name "$worktree_name" --from-config "$source_config_path"
    return 0
  fi
...
write_fallback_worktree_config() {
  ...
  CREWSPACE_DIR="$crewspace_dir" \
  SOURCE_CONFIG_PATH="$source_config_path" \
  SOURCE_ENV_PATH="$source_env_path" \
  CREWSPACE_WORKTREES_DIR="${CREWSPACE_WORKTREES_DIR:-}" \
  node <<'EOF'
...
async function main() {
  ...
  const crewspaceDir = process.env.CREWSPACE_DIR;
  const sourceConfigPath = process.env.SOURCE_CONFIG_PATH;
  const sourceEnvPath = process.env.SOURCE_ENV_PATH;
  const worktreeHome = path.resolve(expandHomePrefix(nonEmpty(process.env.CREWSPACE_WORKTREES_DIR) ?? "~/.crewspace-worktrees"));
  const instanceId = sanitizeInstanceId(worktreeName);
  const instanceRoot = path.resolve(worktreeHome, "instances", instanceId);
  const configPath = path.resolve(crewspaceDir, "config.json");
  const envPath = path.resolve(crewspaceDir, ".env");
  ...
  const targetConfig = {
    ...
    storage: {
      ...
      s3: {
        bucket: sourceConfig?.storage?.s3?.bucket ?? "crewspace",
  ...
  const inlineMasterKey = nonEmpty(sourceEnvEntries.CREWSPACE_SECRETS_MASTER_KEY);
  if (inlineMasterKey) {
    ...
    const sourceKeyFilePath = nonEmpty(sourceEnvEntries.CREWSPACE_SECRETS_MASTER_KEY_FILE)
      ? resolveRuntimeLikePath(sourceEnvEntries.CREWSPACE_SECRETS_MASTER_KEY_FILE, sourceConfigPath)
  ...
  const envLines = [
    "CREWSPACE_HOME=" + JSON.stringify(worktreeHome),
    "CREWSPACE_INSTANCE_ID=" + JSON.stringify(instanceId),
    "CREWSPACE_CONFIG=" + JSON.stringify(configPath),
    "CREWSPACE_CONTEXT=" + JSON.stringify(path.resolve(worktreeHome, "context.json")),
    "CREWSPACE_IN_WORKTREE=true",
    "CREWSPACE_WORKTREE_NAME=" + JSON.stringify(worktreeName),
  ];

  const agentJwtSecret = nonEmpty(sourceEnvEntries.CREWSPACE_AGENT_JWT_SECRET);
  if (agentJwtSecret) {
    envLines.push("CREWSPACE_AGENT_JWT_SECRET=" + JSON.stringify(agentJwtSecret));
  }
...
if ! run_isolated_worktree_init; then
  echo "crewspaceai CLI not available in this workspace; writing isolated fallback config without DB seeding." >&2
  write_fallback_worktree_config
fi
