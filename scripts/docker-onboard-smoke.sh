...
IMAGE_NAME="${IMAGE_NAME:-crewspace-onboard-smoke}"
HOST_PORT="${HOST_PORT:-3131}"
CREWSPACEAI_VERSION="${CREWSPACEAI_VERSION:-latest}"
...
CREWSPACE_DEPLOYMENT_MODE="${CREWSPACE_DEPLOYMENT_MODE:-authenticated}"
CREWSPACE_DEPLOYMENT_EXPOSURE="${CREWSPACE_DEPLOYMENT_EXPOSURE:-private}"
CREWSPACE_PUBLIC_URL="${CREWSPACE_PUBLIC_URL:-http://localhost:${HOST_PORT}}"
SMOKE_AUTO_BOOTSTRAP="${SMOKE_AUTO_BOOTSTRAP:-true}"
SMOKE_ADMIN_NAME="${SMOKE_ADMIN_NAME:-Smoke Admin}"
SMOKE_ADMIN_EMAIL="${SMOKE_ADMIN_EMAIL:-smoke-admin@crewspace.local}"
SMOKE_ADMIN_PASSWORD="${SMOKE_ADMIN_PASSWORD:-crewspace-smoke-password}"
...
write_metadata_file() {
  ...
  mkdir -p "$(dirname "$SMOKE_METADATA_FILE")"
  {
    ...
    printf 'SMOKE_CREWSPACEAI_VERSION=%q\n' "$CREWSPACEAI_VERSION"
  } >"$SMOKE_METADATA_FILE"
}
...
generate_bootstrap_invite_url() {
  ...
  if bootstrap_output="$(
    docker exec \
      -e CREWSPACE_DEPLOYMENT_MODE="$CREWSPACE_DEPLOYMENT_MODE" \
      -e CREWSPACE_DEPLOYMENT_EXPOSURE="$CREWSPACE_DEPLOYMENT_EXPOSURE" \
      -e CREWSPACE_PUBLIC_URL="$CREWSPACE_PUBLIC_URL" \
      -e CREWSPACE_HOME="/crewspace" \
      "$CONTAINER_NAME" bash -lc \
      'timeout 20s npx --yes "crewspaceai@${CREWSPACEAI_VERSION}" auth bootstrap-ceo --data-dir "$CREWSPACE_HOME" --base-url "$CREWSPACE_PUBLIC_URL"' \
      2>&1
  )"; then
    bootstrap_status=0
  else
...
sign_up_or_sign_in() {
  ...
  signup_status="$(post_json_with_cookies \
    "$CREWSPACE_PUBLIC_URL/api/auth/sign-up/email" \
  ...
  signin_status="$(post_json_with_cookies \
    "$CREWSPACE_PUBLIC_URL/api/auth/sign-in/email" \
...
auto_bootstrap_authenticated_smoke() {
  local health_url="$CREWSPACE_PUBLIC_URL/api/health"
  local health_json
  ...
  if [[ "$health_json" == *'"bootstrapStatus":"ready"'* ]]; then
    ...
    accept_status="$(post_json_with_cookies \
      "$CREWSPACE_PUBLIC_URL/api/invites/$invite_token/accept" \
      '{"requestType":"human"}' \
      "$accept_response")"
  ...
  local session_json
  session_json="$(get_with_cookies "$CREWSPACE_PUBLIC_URL/api/auth/get-session")"
  ...
  companies_json="$(get_with_cookies "$CREWSPACE_PUBLIC_URL/api/companies")"
...
docker build \
  --build-arg CREWSPACEAI_VERSION="$CREWSPACEAI_VERSION" \
...
echo "    Public URL: $CREWSPACE_PUBLIC_URL"
echo "    Smoke auto-bootstrap: $SMOKE_AUTO_BOOTSTRAP"
echo "    Detached mode: $SMOKE_DETACH"
echo "    Data dir: $DATA_DIR"
echo "    Deployment: $CREWSPACE_DEPLOYMENT_MODE/$CREWSPACE_DEPLOYMENT_EXPOSURE"
...
docker run -d --rm \
  ...
  -e HOST=0.0.0.0 \
  -e PORT=3100 \
  -e BETTER_AUTH_SECRET="${BETTER_AUTH_SECRET:-crewspace-smoke-auth-secret-32chars!!}" \
  -e CREWSPACE_DEPLOYMENT_MODE="$CREWSPACE_DEPLOYMENT_MODE" \
  -e CREWSPACE_DEPLOYMENT_EXPOSURE="$CREWSPACE_DEPLOYMENT_EXPOSURE" \
  -e CREWSPACE_PUBLIC_URL="$CREWSPACE_PUBLIC_URL" \
  -v "$DATA_DIR:/crewspace" \
  "$IMAGE_NAME" >/dev/null
...
TMP_DIR="$(mktemp -d "${TMPDIR:-/tmp}/crewspace-onboard-smoke.XXXXXX")"
COOKIE_JAR="$TMP_DIR/cookies.txt"

if ! wait_for_http "$CREWSPACE_PUBLIC_URL/api/health" 90 1; then
  echo "Smoke bootstrap failed: server did not become ready at $CREWSPACE_PUBLIC_URL/api/health" >&2
  exit 1
fi

if [[ "$SMOKE_AUTO_BOOTSTRAP" == "true" && "$CREWSPACE_DEPLOYMENT_MODE" == "authenticated" ]]; then
  auto_bootstrap_authenticated_smoke
fi
...
if [[ "$SMOKE_DETACH" == "true" ]]; then
  ...
  echo "    Smoke base URL: $CREWSPACE_PUBLIC_URL"
  echo "    Smoke admin credentials: $SMOKE_ADMIN_EMAIL / $SMOKE_ADMIN_PASSWORD"
