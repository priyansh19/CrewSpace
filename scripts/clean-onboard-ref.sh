...
Environment overrides:
  ...
  PC_DATA=/tmp/data           CrewSpace data dir to use
  CREWSPACE_HOST=127.0.0.1    Host passed to the onboarded server
  CREWSPACE_PORT=3232         Port passed to the onboarded server
...
export PC_TEST_ROOT="${PC_TEST_ROOT:-$(mktemp -d /tmp/crewspace-clean-ref.XXXXXX)}"
export PC_HOME="${PC_HOME:-$PC_TEST_ROOT/home}"
export PC_CACHE="${PC_CACHE:-$PC_TEST_ROOT/npm-cache}"
export PC_DATA="${PC_DATA:-$PC_TEST_ROOT/crewspace-data}"
export PC_REPO="${PC_REPO:-$PC_TEST_ROOT/repo}"
export CREWSPACE_HOST="${CREWSPACE_HOST:-127.0.0.1}"
export CREWSPACE_PORT="${CREWSPACE_PORT:-3100}"
export CREWSPACE_OPEN_ON_LISTEN="${CREWSPACE_OPEN_ON_LISTEN:-false}"
...
echo "CREWSPACE_HOST: $CREWSPACE_HOST"
echo "CREWSPACE_PORT: $CREWSPACE_PORT"
...
env \
  ...
  HOST="$CREWSPACE_HOST" \
  PORT="$CREWSPACE_PORT" \
  CREWSPACE_OPEN_ON_LISTEN="$CREWSPACE_OPEN_ON_LISTEN" \
  pnpm crewspaceai onboard --yes --data-dir "$PC_DATA"
