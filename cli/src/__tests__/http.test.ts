...
import { ApiConnectionError, ApiRequestError, CrewSpaceApiClient } from "../client/http.js";

describe("CrewSpaceApiClient", () => {
  afterEach(() => {
    vi.restoreAllMocks();
  });
  ...
  it("adds authorization and run-id headers", async () => {
    ...
    const client = new CrewSpaceApiClient({
    ...
    expect(headers["x-crewspace-run-id"]).toBe("run-abc");
  ...
  it("returns null on ignoreNotFound", async () => {
    ...
    const client = new CrewSpaceApiClient({ apiBase: "http://localhost:3100" });
