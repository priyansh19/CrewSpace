...
import { ensureAgentJwtSecret, loadCrewSpaceEnvFile, mergeCrewSpaceEnvEntries, readCrewSpaceEnvEntries, resolveCrewSpaceEnvFile } from "../config/env.js";
...
function isCurrentSourceConfigPath(sourceConfigPath: string): boolean {
  const currentConfigPath = process.env.CREWSPACE_CONFIG;
...
const WORKTREE_NAME_PREFIX = "crewspace-";
...
function resolveWorktreeHome(explicit?: string): string {
  return explicit ?? process.env.CREWSPACE_WORKTREES_DIR ?? DEFAULT_WORKTREE_HOME;
}

function resolveWorktreeStartPoint(explicit?: string): string | undefined {
  return explicit ?? nonEmpty(process.env.CREWSPACE_WORKTREE_START_POINT) ?? undefined;
}
...
function createConfiguredStorageFromCrewSpaceConfig(config: CrewSpaceConfig): ConfiguredStorage {
...
function openConfiguredStorage(configPath: string): ConfiguredStorage {
  ...
  return createConfiguredStorageFromCrewSpaceConfig(config);
}
...
function resolveRepoManagedWorktreesRoot(cwd: string): string | null {
  ...
  const marker = `${path.sep}.crewspace${path.sep}worktrees${path.sep}`;
  const index = normalized.indexOf(marker);
  if (index === -1) return null;
  const repoRoot = normalized.slice(0, index);
  return path.resolve(repoRoot, ".crewspace", "worktrees");
}
...
function collectClaimedWorktreePorts(homeDir: string, currentInstanceId: string, cwd: string): {
  ...
  if (repoManagedWorktreesRoot && existsSync(repoManagedWorktreesRoot)) {
    for (const entry of readdirSync(repoManagedWorktreesRoot, { withFileTypes: true })) {
      ...
      const configPath = path.resolve(repoManagedWorktreesRoot, entry.name, ".crewspace", "config.json");
      if (existsSync(configPath)) {
        configPaths.add(configPath);
      }
    }
...
export function resolveSourceConfigPath(opts: WorktreeInitOptions): string {
  ...
  const sourceHome = path.resolve(expandHomePrefix(opts.fromDataDir ?? "~/.crewspace"));
  const sourceInstanceId = sanitizeWorktreeInstanceId(opts.fromInstance ?? "default");
  return path.resolve(sourceHome, "instances", sourceInstanceId, "config.json");
}

function resolveSourceConnectionString(config: CrewSpaceConfig, envEntries: Record<string, string>, portOverride?: number): string {
...
export function copySeededSecretsKey(input: {
  ...
  const sourceInlineMasterKey =
    nonEmpty(input.sourceEnvEntries.CREWSPACE_SECRETS_MASTER_KEY) ??
    (allowProcessEnvFallback ? nonEmpty(process.env.CREWSPACE_SECRETS_MASTER_KEY) : null);
  ...
  const sourceKeyFileOverride =
    nonEmpty(input.sourceEnvEntries.CREWSPACE_SECRETS_MASTER_KEY_FILE) ??
    (allowProcessEnvFallback ? nonEmpty(process.env.CREWSPACE_SECRETS_MASTER_KEY_FILE) : null);
...
async function seedWorktreeDatabase(input: {
  sourceConfigPath: string;
  sourceConfig: CrewSpaceConfig;
  targetConfig: CrewSpaceConfig;
  targetPaths: WorktreeLocalPaths;
  instanceId: string;
  seedMode: WorktreeSeedMode;
}): Promise<SeedWorktreeDatabaseResult> {
  const seedPlan = resolveWorktreeSeedPlan(input.seedMode);
  const sourceEnvFile = resolveCrewSpaceEnvFile(input.sourceConfigPath);
  const sourceEnvEntries = readCrewSpaceEnvEntries(sourceEnvFile);
  ...
  try {
    ...
    const adminConnectionString = `postgres://crewspace:crewspace@127.0.0.1:${targetHandle.port}/postgres`;
    await ensurePostgresDatabase(adminConnectionString, "crewspace");
    const targetConnectionString = `postgres://crewspace:crewspace@127.0.0.1:${targetHandle.port}/crewspace`;
...
async function runWorktreeInit(opts: WorktreeInitOptions): Promise<void> {
  ...
  const sourceEnvEntries = readCrewSpaceEnvEntries(resolveCrewSpaceEnvFile(sourceConfigPath));
  const existingAgentJwtSecret =
    nonEmpty(sourceEnvEntries.CREWSPACE_AGENT_JWT_SECRET) ??
    nonEmpty(process.env.CREWSPACE_AGENT_JWT_SECRET);
  mergeCrewSpaceEnvEntries(
    {
      ...buildWorktreeEnvEntries(paths, branding),
      ...(existingAgentJwtSecret ? { CREWSPACE_AGENT_JWT_SECRET: existingAgentJwtSecret } : {}),
    },
    paths.envPath,
  );
  ensureAgentJwtSecret(paths.configPath);
  loadCrewSpaceEnvFile(paths.configPath);
  ...
  p.outro(
    pc.green(
      `Worktree ready. Run CrewSpace inside this repo and the CLI/server will use ${paths.instanceId} automatically.`,
    ),
  );
}
...
export async function worktreeInitCommand(opts: WorktreeInitOptions): Promise<void> {
  ...
  p.intro(pc.bgCyan(pc.black(" crewspaceai worktree init ")));
  await runWorktreeInit(opts);
}

export async function worktreeMakeCommand(nameArg: string, opts: WorktreeMakeOptions): Promise<void> {
  printCrewSpaceCliBanner();
  p.intro(pc.bgCyan(pc.black(" crewspaceai worktree:make ")));
...
function toMergeSourceChoices(cwd: string): MergeSourceChoice[] {
  ...
  return parseGitWorktreeList(cwd).map((entry) => {
    ...
    return {
      ...
      branch: entry.branch,
      branchLabel,
      hasCrewSpaceConfig: existsSync(path.resolve(worktreePath, ".crewspace", "config.json")),
...
export async function worktreeCleanupCommand(nameArg: string, opts: WorktreeCleanupOptions): Promise<void> {
  printCrewSpaceCliBanner();
  p.intro(pc.bgCyan(pc.black(" crewspaceai worktree:cleanup ")));
...
export async function worktreeEnvCommand(opts: WorktreeEnvOptions): Promise<void> {
  ...
  const envPath = resolveCrewSpaceEnvFile(configPath);
  const envEntries = readCrewSpaceEnvEntries(envPath);
  const out = {
    CREWSPACE_CONFIG: configPath,
    ...(envEntries.CREWSPACE_HOME ? { CREWSPACE_HOME: envEntries.CREWSPACE_HOME } : {}),
    ...(envEntries.CREWSPACE_INSTANCE_ID ? { CREWSPACE_INSTANCE_ID: envEntries.CREWSPACE_INSTANCE_ID } : {}),
    ...(envEntries.CREWSPACE_CONTEXT ? { CREWSPACE_CONTEXT: envEntries.CREWSPACE_CONTEXT } : {}),
    ...envEntries,
  };
...
function resolveAttachmentLookupStorages(input: {
  ...
  const orderedConfigPaths = [
    ...
    ...toMergeSourceChoices(process.cwd())
      .filter((choice) => choice.hasCrewSpaceConfig)
      .map((choice) => path.resolve(choice.worktree, ".crewspace", "config.json")),
  ];
...
async function openConfiguredDb(configPath: string): Promise<OpenDbHandle> {
  ...
  const envEntries = readCrewSpaceEnvEntries(resolveCrewSpaceEnvFile(configPath));
    ...
    if (migrationState.status !== "upToDate") {
      const pending =
        migrationState.reason === "pending-migrations"
          ...
          : "";
      throw new Error(
        `Database for ${configPath} is not up to date.${pending} Run \`pnpm db:migrate\` (or start CrewSpace once) before using worktree merge history.`,
      );
    }
...
export async function worktreeListCommand(opts: WorktreeListOptions): Promise<void> {
  ...
  for (const choice of choices) {
    const flags = [
      ...
      choice.hasCrewSpaceConfig ? "crewspace" : "no-crewspace-config",
...
function resolveEndpointFromChoice(choice: MergeSourceChoice): ResolvedWorktreeEndpoint {
  ...
  return {
    ...
    configPath: path.resolve(choice.worktree, ".crewspace", "config.json"),
...
function resolveWorktreeEndpointFromSelector(
  ...
  if (existsSync(directPath)) {
    ...
    const configPath = path.resolve(directPath, ".crewspace", "config.json");
    if (!existsSync(configPath)) {
      throw new Error(`Resolved worktree path ${directPath} does not contain .crewspace/config.json.`);
    }
    return {
      rootPath: directPath,
      configPath,
  ...
  if (!matched.hasCrewSpaceConfig && !matched.isCurrent) {
    throw new Error(`Resolved worktree "${selector}" does not look like a CrewSpace worktree.`);
  }
...
async function promptForSourceEndpoint(excludeWorktreePath?: string): Promise<ResolvedWorktreeEndpoint> {
  ...
  const currentEndpoint = resolveCurrentEndpoint();
  const choices = toMergeSourceChoices(process.cwd())
    .filter((choice) => choice.hasCrewSpaceConfig || choice.isCurrent)
  ...
  if (choices.length === 0) {
    throw new Error("No CrewSpace worktrees were found. Run `crewspaceai worktree:list` to inspect the repo worktrees.");
  }
...
export async function worktreeMergeHistoryCommand(sourceArg: string | undefined, opts: WorktreeMergeHistoryOptions): Promise<void> {
  ...
  if (path.resolve(sourceEndpoint.configPath) === path.resolve(targetEndpoint.configPath)) {
    throw new Error("Source and target CrewSpace configs are the same. Choose different --from/--to worktrees.");
  }
...
export function registerWorktreeCommands(program: Command): void {
  const worktree = program.command("worktree").description("Worktree-local CrewSpace instance helpers");

  program
    .command("worktree:make")
    .description("Create ~/NAME as a git worktree, then initialize an isolated CrewSpace instance inside it")
    .argument("<name>", "Worktree name — auto-prefixed with crewspace- if needed (created at ~/crewspace-NAME)")
    .option("--start-point <ref>", "Remote ref to base the new branch on (env: CREWSPACE_WORKTREE_START_POINT)")
    .option("--instance <id>", "Explicit isolated instance id")
    .option("--home <path>", `Home root for worktree instances (env: CREWSPACE_WORKTREES_DIR, default: ${DEFAULT_WORKTREE_HOME})`)
    .option("--from-config <path>", "Source config.json to seed from")
    .option("--from-data-dir <path>", "Source CREWSPACE_HOME used when deriving the source config")
  ...
  worktree
    .command("env")
    .description("Print shell exports for the current worktree-local CrewSpace instance")
  ...
  program
    ...
    .description("List git worktrees visible from this repo and whether they look like CrewSpace worktrees")
  ...
  program
    ...
    .argument("<name>", "Worktree name — auto-prefixed with crewspace- if needed")
