...
import { formatDatabaseBackupResult, runDatabaseBackup } from "@crewspaceai/db";
import {
  expandHomePrefix,
...
function resolveConnectionString(configPath?: string): { value: string; source: string } {
  ...
  return {
    value: `postgres://crewspace:crewspace@127.0.0.1:${port}/crewspace`,
...
export async function dbBackupCommand(opts: DbBackupOptions): Promise<void> {
  ...
  p.intro(pc.bgCyan(pc.black(" crewspace db:backup ")));

  const configPath = resolveConfigPath(opts.config);
  const config = readConfig(opts.config);
  const connection = resolveConnectionString(opts.config);
  const defaultDir = resolveDefaultBackupDir(resolveCrewSpaceInstanceId());
  ...
  const filenamePrefix = opts.filenamePrefix?.trim() || "crewspace";
