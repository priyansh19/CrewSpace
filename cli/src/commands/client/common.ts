...
import { ApiRequestError, CrewSpaceApiClient } from "../../client/http.js";
...
export function addCommonClientOptions(command: Command, opts?: { includeCompany?: boolean }): Command {
  command
    .option("-c, --config <path>", "Path to CrewSpace config file")
    .option("-d, --data-dir <path>", "CrewSpace data directory root (isolates state from ~/.crewspace)")
    .option("--context <path>", "Path to CLI context file")
    .option("--profile <name>", "CLI context profile name")
    .option("--api-base <url>", "Base URL for the CrewSpace API")
...
export function resolveCommandContext(
  ...
  const apiBase =
    ...
    process.env.CREWSPACE_API_URL?.trim() ||
    profile.apiBase ||
    inferApiBaseFromConfig(options.config);

  const explicitApiKey =
    options.apiKey?.trim() ||
    process.env.CREWSPACE_API_KEY?.trim() ||
  ...
  const companyId =
    ...
    process.env.CREWSPACE_COMPANY_ID?.trim() ||
    profile.companyId;

  if (opts?.requireCompany && !companyId) {
    throw new Error(
      "Company ID is required. Pass --company-id, set CREWSPACE_COMPANY_ID, or set context profile companyId via `crewspaceai context set`.",
    );
  }

  const api = new CrewSpaceApiClient({
    apiBase,
    apiKey,
...
function inferApiBaseFromConfig(configPath?: string): string {
  const envHost = process.env.CREWSPACE_SERVER_HOST?.trim() || "localhost";
  let port = Number(process.env.CREWSPACE_SERVER_PORT || "");
