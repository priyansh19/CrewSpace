...
function buildAgentEnvExports(input: {
  ...
  return [
    `export CREWSPACE_API_URL='${escaped(input.apiBase)}'`,
    `export CREWSPACE_COMPANY_ID='${escaped(input.companyId)}'`,
    `export CREWSPACE_AGENT_ID='${escaped(input.agentId)}'`,
    `export CREWSPACE_API_KEY='${escaped(input.apiKey)}'`,
  ].join("\n");
}
...
export function registerAgentCommands(program: Command): void {
  ...
  addCommonClientOptions(
    agent
      .command("local-cli")
      .description(
        "Create an agent API key, install local CrewSpace skills for Codex/Claude, and print shell exports",
      )
      ...
      .option(
        "--no-install-skills",
        "Skip installing CrewSpace skills into ~/.codex/skills and ~/.claude/skills",
      )
      .action(async (agentRef: string, opts: AgentLocalCliOptions) => {
        try {
          ...
          // Revoke any existing non-revoked keys with the same name to prevent duplication
          const existingKeys = await ctx.api.get<Array<{ id: string; name: string; revokedAt: string | null }>>(
            `/api/agents/${agentRow.id}/keys`
          );
          if (existingKeys) {
            for (const existing of existingKeys) {
              if (existing.name === keyName && !existing.revokedAt) {
                await ctx.api.delete(`/api/agents/${agentRow.id}/keys/${existing.id}`).catch(() => {});
              }
            }
          }
          ...
          if (opts.installSkills !== false) {
            const skillsDir = await resolveCrewSpaceSkillsDir(__moduleDir, [path.resolve(process.cwd(), "skills")]);
            if (!skillsDir) {
              throw new Error(
                "Could not locate local CrewSpace skills directory. Expected ./skills in the repo checkout.",
              );
            }
