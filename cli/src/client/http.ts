...
export class CrewSpaceApiClient {
  ...
  private async request<T>(
    ...
    if (this.runId) {
      headers["x-crewspace-run-id"] = this.runId;
    }
...
function buildConnectionErrorMessage(input: {
  ...
  const lines = [
    "Could not reach the CrewSpace API.",
    "",
  ...
  lines.push(
    "",
    "This usually means the CrewSpace server is not running, the configured URL is wrong, or the request is being blocked before it reaches CrewSpace.",
    "",
    "Try:",
    "- Start CrewSpace with `pnpm dev` or `pnpm crewspaceai run`.",
    `- Verify the server is reachable with \`curl ${healthUrl}\`.`,
    `- If CrewSpace is running elsewhere, pass \`--api-base ${input.apiBase.replace(/\/+$/, "")}\` or set \`CREWSPACE_API_URL\`.`,
  );
  return lines.join("\n");
}
