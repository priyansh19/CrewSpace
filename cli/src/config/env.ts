...
const JWT_SECRET_ENV_KEY = "CREWSPACE_AGENT_JWT_SECRET";
...
function renderEnvFile(entries: Record<string, string>) {
  const lines = [
    ...
    "# Generated by CrewSpace CLI commands",
...
export function resolveCrewSpaceEnvFile(configPath?: string): string {
...
export function loadCrewSpaceEnvFile(configPath?: string): void {
...
export function writeAgentJwtEnv(secret: string, filePath = resolveEnvFilePath()): void {
  mergeCrewSpaceEnvEntries({ [JWT_SECRET_ENV_KEY]: secret }, filePath);
}

export function readCrewSpaceEnvEntries(filePath = resolveEnvFilePath()): Record<string, string> {
  if (!fs.existsSync(filePath)) return {};
  return parseEnvFile(fs.readFileSync(filePath, "utf-8"));
}

export function writeCrewSpaceEnvEntries(entries: Record<string, string>, filePath = resolveEnvFilePath()): void {
...
export function mergeCrewSpaceEnvEntries(
  entries: Record<string, string>,
  filePath = resolveEnvFilePath(),
): Record<string, string> {
  const current = readCrewSpaceEnvEntries(filePath);
  const next = {
    ...current,
    ...Object.fromEntries(
  ...
  writeCrewSpaceEnvEntries(next, filePath);
  return next;
}
