/**
 * esbuild configuration for building the crewspaceai CLI for npm.
 *
 * Bundles all workspace packages (@crewspaceai/*) into a single file.
...
// to npm and resolved at runtime (e.g. @crewspaceai/server uses dynamic import).
...
for (const p of workspacePaths) {
  ...
  for (const name of Object.keys(pkg.dependencies || {})) {
    if (externalWorkspacePackages.has(name)) {
      externals.add(name);
    } else if (!name.startsWith("@crewspaceai/")) {
      externals.add(name);
    }
  }
