...
import type { CrewSpaceConfig } from "@crewspaceai/shared";
...
function formatEnvEntries(entries: Record<string, string>): string {
  return [
    ...
    "# Generated by CrewSpace worktree repair",
...
function resolveWorktreeRuntimeContext(
  ...
  if (env.CREWSPACE_IN_WORKTREE !== "true") return null;

  const configPath = resolveCrewSpaceConfigPath(overrideConfigPath);
  const envPath = resolveCrewSpaceEnvPath(configPath);
  const worktreeRoot = path.resolve(path.dirname(configPath), "..");
  const worktreeName = nonEmpty(env.CREWSPACE_WORKTREE_NAME) ?? path.basename(worktreeRoot);
  const instanceId = nonEmpty(env.CREWSPACE_INSTANCE_ID) ?? sanitizeWorktreeInstanceId(worktreeName);
  const homeDir = resolveHomeAwarePath(
    nonEmpty(env.CREWSPACE_HOME) ??
      nonEmpty(env.CREWSPACE_WORKTREES_DIR) ??
      "~/.crewspace-worktrees",
  );
...
function writeConfigFile(configPath: string, config: CrewSpaceConfig): void {
...
function collectSiblingWorktreePorts(context: WorktreeRuntimeContext): {
  ...
  if (repoManagedWorktreesRoot && fs.existsSync(repoManagedWorktreesRoot)) {
    for (const entry of fs.readdirSync(repoManagedWorktreesRoot, { withFileTypes: true })) {
      ...
      const siblingConfigPath = path.resolve(repoManagedWorktreesRoot, entry.name, ".crewspace", "config.json");
  ...
  for (const siblingConfigPath of siblingConfigPaths) {
    try {
      const siblingConfig = JSON.parse(fs.readFileSync(siblingConfigPath, "utf8")) as CrewSpaceConfig;
...
function buildIsolatedWorktreeConfig(
  ...
  const databasePort =
    config.database.mode === "embedded-postgres"
      ...
      : undefined;
  const nextConfig: CrewSpaceConfig = {
    ...config,
    database: {
      ...config.database,
      ...(config.database.mode === "embedded-postgres"
...
): { config: CrewSpaceConfig; changed: boolean } {
  let changed = false;
...
export function maybeRepairLegacyWorktreeConfigAndEnvFiles(): {
  ...
  process.env.CREWSPACE_HOME = context.homeDir;
  process.env.CREWSPACE_INSTANCE_ID = context.instanceId;
  process.env.CREWSPACE_CONFIG = context.configPath;
  process.env.CREWSPACE_CONTEXT = context.contextPath;
  process.env.CREWSPACE_WORKTREE_NAME = context.worktreeName;

  let repairedConfig = false;
  if (fs.existsSync(context.configPath)) {
    try {
      const parsed = JSON.parse(fs.readFileSync(context.configPath, "utf8")) as CrewSpaceConfig;
  ...
  const desiredEnvEntries: Record<string, string> = {
    ...existingEnvEntries,
    CREWSPACE_HOME: context.homeDir,
    CREWSPACE_INSTANCE_ID: context.instanceId,
    CREWSPACE_CONFIG: context.configPath,
    CREWSPACE_CONTEXT: context.contextPath,
    CREWSPACE_IN_WORKTREE: "true",
    CREWSPACE_WORKTREE_NAME: context.worktreeName,
  };
...
export function initializeWorktreeEnvAndConfig(input: {
  worktreePath: string;
  branchName: string;
}): void {
  const worktreeHome = resolveHomeAwarePath(
    nonEmpty(process.env.CREWSPACE_WORKTREES_DIR) ?? "~/.crewspace-worktrees",
  );
  const instanceId = sanitizeWorktreeInstanceId(input.branchName);
  const instanceRoot = path.resolve(worktreeHome, "instances", instanceId);
  const worktreeConfigPath = path.resolve(input.worktreePath, ".crewspace", "config.json");
  const worktreeEnvPath = path.resolve(input.worktreePath, ".crewspace", ".env");
  const contextPath = path.resolve(worktreeHome, "context.json");

  const context: WorktreeRuntimeContext = {
    configPath: worktreeConfigPath,
    envPath: worktreeEnvPath,
    worktreeName: input.branchName,
    instanceId,
  ...
  const parentConfigPath = resolveCrewSpaceConfigPath();
  if (fs.existsSync(parentConfigPath)) {
    try {
      const parentConfig = JSON.parse(fs.readFileSync(parentConfigPath, "utf8")) as CrewSpaceConfig;
      const siblingPorts = collectSiblingWorktreePorts(context);
      const selectedServerPort = findNextUnclaimedPort(
        parentConfig.server.port === 3100 ? 3101 : parentConfig.server.port,
        siblingPorts.serverPorts,
      );
      const selectedDatabasePort =
        parentConfig.database.mode === "embedded-postgres"
          ? findNextUnclaimedPort(
              parentConfig.database.embeddedPostgresPort === 54329
                ? 54330
                : parentConfig.database.embeddedPostgresPort,
              new Set([...siblingPorts.databasePorts, selectedServerPort]),
            )
          : undefined;
      writeConfigFile(
        worktreeConfigPath,
        buildIsolatedWorktreeConfig(parentConfig, context, {
          serverPort: selectedServerPort,
          databasePort: selectedDatabasePort,
        }),
      );
    } catch {
      // Leave config init to the worktree startup path if parent config is unavailable.
    }
  }

  // DATABASE_URL is intentionally excluded so each worktree uses its own
  // embedded-postgres instance rather than sharing the parent's database.
  const envContent = [
    "# CrewSpace environment variables",
    "# Generated by CrewSpace worktree initialization",
    `CREWSPACE_HOME=${JSON.stringify(worktreeHome)}`,
    `CREWSPACE_INSTANCE_ID=${JSON.stringify(instanceId)}`,
    `CREWSPACE_CONFIG=${JSON.stringify(worktreeConfigPath)}`,
    `CREWSPACE_CONTEXT=${JSON.stringify(contextPath)}`,
    `CREWSPACE_IN_WORKTREE=true`,
    `CREWSPACE_WORKTREE_NAME=${JSON.stringify(input.branchName)}`,
    "",
  ].join("\n");
  fs.mkdirSync(path.dirname(worktreeEnvPath), { recursive: true });
  fs.writeFileSync(worktreeEnvPath, envContent, { mode: 0o600 });
}
...
export function maybePersistWorktreeRuntimePorts(input: {
  ...
  let fileConfig: CrewSpaceConfig;
  try {
    fileConfig = JSON.parse(fs.readFileSync(context.configPath, "utf8")) as CrewSpaceConfig;
  } catch {
    return;
  }
