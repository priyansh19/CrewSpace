...
import { pluginLogs, agentTaskSessions as agentTaskSessionsTable } from "@crewspaceai/db";
 ...
 *
 * This implementation delegates to the core CrewSpace domain services,
  ...
  return {
    ...
    events: {
      ...
      async subscribe(params: { eventPattern: string; filter?: Record<string, unknown> | null }) {
        const handler = async (event: import("@crewspaceai/plugin-sdk").PluginEvent) => {
          if (notifyWorker) {
