...
const DEFERRED_WAKE_CONTEXT_KEY = "_crewspaceWakeContext";
...
export function heartbeatService(db: Db) {
  ...
  async function executeRun(runId: string) {
    ...
    context.crewspaceWorkspaces = resolvedWorkspace.workspaceHints;
    ...
    if (runtimeServiceIntents.length > 0) {
      context.crewspaceRuntimeServiceIntents = runtimeServiceIntents;
    } else {
      delete context.crewspaceRuntimeServiceIntents;
    }
    ...
    if (sessionCompaction.rotate) {
      context.crewspaceSessionHandoffMarkdown = sessionCompaction.handoffMarkdown;
      context.crewspaceSessionRotationReason = sessionCompaction.reason;
      context.crewspacePreviousSessionId = previousSessionDisplayId ?? runtimeSessionIdForAdapter;
      ...
      delete context.crewspaceSessionHandoffMarkdown;
      delete context.crewspaceSessionRotationReason;
      delete context.crewspacePreviousSessionId;
    }
    ...
    try {
      ...
      if (runtimeServices.length > 0) {
        context.crewspaceRuntimeServices = runtimeServices;
        context.crewspaceRuntimePrimaryUrl =
      ...
      if (issueId && (executionWorkspace.created || runtimeServices.some((service) => !service.reused))) {
        try {
          ...
          await onLog(
            "stderr",
            `[crewspace] Failed to post workspace-ready comment: ${err instanceof Error ? err.message : String(err)}\n`,
          );
        }
      }
      ...
      if (adapter.supportsLocalAgentJwt && !authToken) {
        logger.warn(
          ...
          "local agent jwt secret missing or invalid; running without injected CREWSPACE_API_KEY",
        );
      }
      ...
      if (adapterManagedRuntimeServices.length > 0) {
        ...
        context.crewspaceRuntimeServices = combinedRuntimeServices;
        context.crewspaceRuntimePrimaryUrl =
        ...
        if (issueId) {
          try {
            ...
            await onLog(
              "stderr",
              `[crewspace] Failed to post adapter-managed runtime comment: ${err instanceof Error ? err.message : String(err)}\n`,
            );
          }
        }
      }
