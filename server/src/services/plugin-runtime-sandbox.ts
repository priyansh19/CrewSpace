...
import type { CrewSpacePluginManifestV1 } from "@crewspaceai/shared";
...
export async function loadPluginModuleInSandbox(
  ...
  const loadModuleSync = (modulePath: string): Record<string, unknown> => {
    ...
    const sandboxArgs = {
      __crewspace_exports: module.exports,
      __crewspace_module: module,
      __crewspace_require: requireInSandbox,
      __crewspace_filename: realPath,
      __crewspace_dirname: path.dirname(realPath),
    };
    // Temporarily inject args into the context, run, then remove to avoid pollution.
    Object.assign(context, sandboxArgs);
    const wrapped = `(function (exports, module, require, __filename, __dirname) {\n${code}\n})(__crewspace_exports, __crewspace_module, __crewspace_require, __crewspace_filename, __crewspace_dirname)`;
