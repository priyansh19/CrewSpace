 ...
 */
import { pluginManifestV1Schema } from "@crewspaceai/shared";
import type { CrewSpacePluginManifestV1 } from "@crewspaceai/shared";
import { PLUGIN_API_VERSION } from "@crewspaceai/shared";
...
export interface PluginManifestValidator {
   ...
   */
  parseOrThrow(input: unknown): CrewSpacePluginManifestV1;
  ...
  return {
    parse(input: unknown): ManifestParseResult {
      ...
      if (result.success) {
        return {
          success: true,
          manifest: result.data as CrewSpacePluginManifestV1,
        };
      }
