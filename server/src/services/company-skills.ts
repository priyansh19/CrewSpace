...
import { companySkills } from "@crewspaceai/db";
import { readCrewSpaceSkillSyncPreference, writeCrewSpaceSkillSyncPreference } from "@crewspaceai/adapter-utils/server-utils";
import type { CrewSpaceSkillEntry } from "@crewspaceai/adapter-utils/server-utils";
import type {
  CompanySkill,
...
import { normalizeAgentUrlKey } from "@crewspaceai/shared";
import { findServerAdapter } from "../adapters/index.js";
import { resolveCrewSpaceInstanceRoot } from "../home-paths.js";
...
function buildSkillRuntimeName(key: string, slug: string) {
  if (key.startsWith("crewspaceai/crewspace/")) return slug;
...
function resolveManagedSkillsRoot(companyId: string) {
  return path.resolve(resolveCrewSpaceInstanceRoot(), "skills", companyId);
}
...
function deriveSkillSourceInfo(skill: CompanySkill): {
  ...
  if (metadata.sourceKind === "crewspace_bundled") {
    return {
      editable: false,
      editableReason: "Bundled CrewSpace skills are read-only.",
      sourceLabel: "CrewSpace bundled",
      sourceBadge: "crewspace",
      sourcePath: null,
    };
  }
  ...
  if (skill.sourceType === "local_path") {
    ...
    if (localSkillDir && localSkillDir.startsWith(managedRoot)) {
      return {
        editable: true,
        editableReason: null,
        sourceLabel: "CrewSpace workspace",
        sourceBadge: "crewspace",
...
export function companySkillService(db: Db) {
  ...
  ): Promise<CrewSpaceSkillEntry[]> {
    const skills = await listFull(companyId);

    const out: CrewSpaceSkillEntry[] = [];
    for (const skill of skills) {
      ...
      const required = sourceKind === "crewspace_bundled";
      out.push({
        key: skill.key,
        ...
        requiredReason: required
          ? "Bundled CrewSpace skills are always available for local adapters."
          : null,
      });
    }
  ...
  async function upsertImportedSkills(companyId: string, imported: ImportedSkill[]): Promise<CompanySkill[]> {
    ...
    for (const skill of imported) {
      ...
      if (
        existing
        && existingMeta.sourceKind === "crewspace_bundled"
        && incomingKind === "github"
        && incomingOwner === "crewspaceai"
        && incomingRepo === "crewspace"
      ) {
        out.push(existing);
        continue;
      }
    ...
    for (const agent of agentRows) {
      ...
      if (referencesSkill) {
        ...
        await agents.update(agent.id, {
          adapterConfig: writeCrewSpaceSkillSyncPreference(config, filtered),
        });
      }
    }
