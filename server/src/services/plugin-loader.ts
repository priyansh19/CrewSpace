 ...
 *    (`~/.crewspace/plugins/`) and `node_modules` for packages matching
 *    the `crewspace-plugin-*` naming convention. Aggregates results with
...
/**
 * Naming convention for npm-published CrewSpace plugins.
 * Packages matching this pattern are considered CrewSpace plugins.
 *
 * @see PLUGIN_SPEC.md §10 — Package Contract
 */
export const NPM_PLUGIN_PACKAGE_PREFIX = "crewspace-plugin-";
...
export interface DiscoveredPlugin {
  ...
  manifest: CrewSpacePluginManifestV1 | null;
}
...
export type PluginSource =
  | "local-filesystem"  // ~/.crewspace/plugins/ local directory
  | "npm"               // npm packages matching crewspace-plugin-* convention
...
function getDeclaredPageRoutePaths(manifest: CrewSpacePluginManifestV1): string[] {
...
export interface PluginLoaderOptions {
  ...
  /**
   * Whether to discover installed npm packages matching the crewspace-plugin-*
   * naming convention.
   * Defaults to true.
   */
...
export interface PluginInstallOptions {
  /**
   * npm package name to install (e.g. "crewspace-plugin-linear" or "@acme/plugin-linear").
...
export interface PluginRuntimeServices {
   ...
   */
  buildHostHandlers: (pluginId: string, manifest: CrewSpacePluginManifestV1) => WorkerToHostHandlers;
  /**
...
export interface PluginLoader {
  ...
  /**
   * Discover CrewSpace plugins installed as npm packages in the current
   * Node.js environment matching the "crewspace-plugin-*" naming convention.
   *
   ...
   * the "crewspacePlugin.manifest" key, loads the manifest module, and
   * validates it against the plugin manifest schema.
   *
   * Returns null if the package is not a CrewSpace plugin.
   * Throws if the package is a CrewSpace plugin but the manifest is invalid.
   *
   * @see PLUGIN_SPEC.md §10 — Package Contract
   */
  loadManifest(packagePath: string): Promise<CrewSpacePluginManifestV1 | null>;
  ...
  upgradePlugin(pluginId: string, options: Omit<PluginInstallOptions, "installDir">): Promise<{
    oldManifest: CrewSpacePluginManifestV1;
    newManifest: CrewSpacePluginManifestV1;
    discovered: DiscoveredPlugin;
  }>;
...
/**
 * Check whether a package name matches the CrewSpace plugin naming convention.
 * Accepts both the "crewspace-plugin-" prefix and scoped "@scope/plugin-" packages.
 *
 * @see PLUGIN_SPEC.md §10 — Package Contract
 */
export function isPluginPackageName(name: string): boolean {
  if (name.startsWith(NPM_PLUGIN_PACKAGE_PREFIX)) return true;
  // Also accept scoped packages like @acme/plugin-linear or @crewspaceai/plugin-*
  if (name.includes("/")) {
 ...
 *
 * The spec defines a "crewspacePlugin" key in package.json with a "manifest"
...
function resolveManifestPath(
  ...
  const crewspacePlugin = pkgJson["crewspacePlugin"];
  if (
    crewspacePlugin !== null &&
    typeof crewspacePlugin === "object" &&
    !Array.isArray(crewspacePlugin)
  ) {
    const manifestRelPath = (crewspacePlugin as Record<string, unknown>)[
      "manifest"
    ];
...
function getMinimumHostVersion(manifest: CrewSpacePluginManifestV1): string | undefined {
  return manifest.minimumHostVersion ?? manifest.minimumCrewSpaceVersion;
}
 ...
 *   packageName: "crewspace-plugin-linear",
 *   version: "^1.0.0",
 * });
 * ```
 *
...
export function pluginLoader(
  ...
  async function assertPageRoutePathsAvailable(manifest: CrewSpacePluginManifestV1): Promise<void> {
    ...
    for (const plugin of installedPlugins) {
      ...
      const installedManifest = plugin.manifestJson as CrewSpacePluginManifestV1 | null;
  ...
  async function fetchAndValidate(
    ...
    if (!manifestPath || !existsSync(manifestPath)) {
      throw new Error(
        `Package ${resolvedPackageName} at ${resolvedPackagePath} does not appear to be a CrewSpace plugin (no manifest found).`,
      );
    }
  ...
  async function loadManifestFromPath(
    manifestPath: string,
  ): Promise<CrewSpacePluginManifestV1> {
    let raw: unknown;
   ...
   * if the package is not a CrewSpace plugin.
   */
  async function buildDiscoveredPlugin(
    ...
    const hasCrewSpacePlugin = "crewspacePlugin" in pkgJson;
    const nameMatchesConvention = isPluginPackageName(packageName);

    if (!hasCrewSpacePlugin && !nameMatchesConvention) {
      return null;
    }
  ...
  return {
    ...
    async loadManifest(packagePath: string): Promise<CrewSpacePluginManifestV1 | null> {
      const pkgJson = await readPackageJson(packagePath);
      if (!pkgJson) return null;
    ...
    async upgradePlugin(
      ...
      const plugin = (await registry.getById(pluginId)) as {
        ...
        manifestJson: CrewSpacePluginManifestV1;
      } | null;
  ...
  async function activatePlugin(plugin: PluginRecord): Promise<PluginLoadResult> {
    ...
    try {
      ...
      // (for example @crewspaceai/shared exports). Run those workers through
