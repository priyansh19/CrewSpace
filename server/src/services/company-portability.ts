...
function classifyPortableFileKind(pathValue: string): CompanyPortabilityExportPreviewResult["fileInventory"][number]["kind"] {
  ...
  if (normalized === ".crewspace.yaml" || normalized === ".crewspace.yml") return "extension";
...
function readSkillKey(frontmatter: Record<string, unknown>) {
  ...
  const crewspace = isPlainRecord(metadata?.crewspace) ? metadata?.crewspace as Record<string, unknown> : null;
  return normalizeSkillKey(
    asString(frontmatter.key)
    ?? asString(frontmatter.skillKey)
    ?? asString(metadata?.skillKey)
    ?? asString(metadata?.canonicalKey)
    ?? asString(metadata?.crewspaceSkillKey)
    ?? asString(crewspace?.skillKey)
    ?? asString(crewspace?.key),
  );
}
...
function deriveManifestSkillKey(
  ...
  if (sourceKind === "crewspace_bundled") {
    return `crewspaceai/crewspace/${slug}`;
  }
...
const ADAPTER_DEFAULT_RULES_BY_TYPE: Record<string, Array<{ path: string[]; value: unknown }>> = {
  ...
  openclaw_gateway: [
    ...
    { path: ["sessionKey"], value: "crewspace" },
...
function buildLegacyRoutineTriggerFromRecurrence(
  ...
  if (!frequency) {
    errors.push(`Recurring task ${issue.slug} uses legacy recurrence without frequency; add .crewspace.yaml routines.${issue.slug}.triggers.`);
    return { trigger: null, warnings, errors };
  }
  if (interval < 1) {
    errors.push(`Recurring task ${issue.slug} uses legacy recurrence with an invalid interval; add .crewspace.yaml routines.${issue.slug}.triggers.`);
    return { trigger: null, warnings, errors };
  }
  ...
  if (startsAt && !zonedStartsAt) {
    errors.push(`Recurring task ${issue.slug} has an invalid legacy startsAt/timezone combination; add .crewspace.yaml routines.${issue.slug}.triggers.`);
    return { trigger: null, warnings, errors };
  }
  ...
  if (hour < 0 || hour > 23 || minute < 0 || minute > 59) {
    errors.push(`Recurring task ${issue.slug} uses legacy recurrence with an invalid time; add .crewspace.yaml routines.${issue.slug}.triggers.`);
    return { trigger: null, warnings, errors };
  }

  if (issue.legacyRecurrence.until != null || issue.legacyRecurrence.count != null) {
    warnings.push(`Recurring task ${issue.slug} uses legacy recurrence end bounds; CrewSpace will import the routine trigger without those limits.`);
  }
  ...
  if (frequency === "hourly") {
    ...
    if (Array.isArray(issue.legacyRecurrence.weekdays) || Array.isArray(issue.legacyRecurrence.monthDays) || Array.isArray(issue.legacyRecurrence.months)) {
      errors.push(`Recurring task ${issue.slug} uses unsupported legacy daily recurrence constraints; add .crewspace.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    ...
    if (interval !== 1) {
      errors.push(`Recurring task ${issue.slug} uses legacy weekly recurrence with interval > 1; add .crewspace.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    ...
    if (cronWeekdays.length === 0) {
      errors.push(`Recurring task ${issue.slug} uses legacy weekly recurrence without weekdays; add .crewspace.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    cronExpression = `${minute} ${hour} * * ${normalizeCronList(cronWeekdays)}`;
  } else if (frequency === "monthly") {
    if (interval !== 1) {
      errors.push(`Recurring task ${issue.slug} uses legacy monthly recurrence with interval > 1; add .crewspace.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    if (Array.isArray(issue.legacyRecurrence.ordinalWeekdays) && issue.legacyRecurrence.ordinalWeekdays.length > 0) {
      errors.push(`Recurring task ${issue.slug} uses legacy ordinal monthly recurrence; add .crewspace.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    ...
    if (monthDays.length === 0) {
      errors.push(`Recurring task ${issue.slug} uses legacy monthly recurrence without monthDays; add .crewspace.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    ...
    if (interval !== 1) {
      errors.push(`Recurring task ${issue.slug} uses legacy yearly recurrence with interval > 1; add .crewspace.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    const months = Array.isArray(issue.legacyRecurrence.months)
      ? issue.legacyRecurrence.months
    ...
    if (months.length === 0 || monthDays.length === 0) {
      errors.push(`Recurring task ${issue.slug} uses legacy yearly recurrence without month/monthDay anchors; add .crewspace.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    cronExpression = `${minute} ${hour} ${normalizeCronList(monthDays.map(String))} ${normalizeCronList(months.map(String))} *`;
  } else {
    errors.push(`Recurring task ${issue.slug} uses unsupported legacy recurrence frequency "${frequency}"; add .crewspace.yaml routines.${issue.slug}.triggers.`);
    return { trigger: null, warnings, errors };
  }
...
function filterExportFiles(
  ...
  const extensionEntry = filtered[crewspaceExtensionPath];
  if (selectedFiles.has(crewspaceExtensionPath) && typeof extensionEntry === "string") {
    filtered[crewspaceExtensionPath] = filterPortableExtensionYaml(extensionEntry, selectedFiles);
  }

  return filtered;
}

function findCrewSpaceExtensionPath(files: Record<string, CompanyPortabilityFileEntry>) {
  if (typeof files[".crewspace.yaml"] === "string") return ".crewspace.yaml";
  if (typeof files[".crewspace.yml"] === "string") return ".crewspace.yml";
  return Object.keys(files).find((entry) => entry.endsWith("/.crewspace.yaml") || entry.endsWith("/.crewspace.yml")) ?? null;
}
...
async function buildSkillSourceEntry(skill: CompanySkill) {
  const metadata = isPlainRecord(skill.metadata) ? skill.metadata : null;
  if (asString(metadata?.sourceKind) === "crewspace_bundled") {
    const commit = await resolveBundledSkillsCommit();
    return {
      kind: "github-dir",
      repo: "crewspaceai/crewspace",
      path: `skills/${skill.slug}`,
      commit,
      trackingRef: "master",
      url: `https://github.com/crewspaceai/crewspace/tree/master/skills/${skill.slug}`,
    };
  }
...
function shouldReferenceSkillOnExport(skill: CompanySkill, expandReferencedSkills: boolean) {
  ...
  const metadata = isPlainRecord(skill.metadata) ? skill.metadata : null;
  if (asString(metadata?.sourceKind) === "crewspace_bundled") return true;
...
async function withSkillSourceMetadata(skill: CompanySkill, markdown: string) {
  ...
  metadata.crewspaceSkillKey = skill.key;
  metadata.crewspace = {
    ...(isPlainRecord(metadata.crewspace) ? metadata.crewspace : {}),
    skillKey: skill.key,
    slug: skill.slug,
  };
  const frontmatter = {
...
function buildManifestFromPackageFiles(
  ...
  const crewspaceExtensionPath = findCrewSpaceExtensionPath(normalizedFiles);
  const crewspaceExtension = crewspaceExtensionPath
    ? parseYamlFile(readPortableTextFile(normalizedFiles, crewspaceExtensionPath) ?? "")
    : {};
  const crewspaceCompany = isPlainRecord(crewspaceExtension.company) ? crewspaceExtension.company : {};
  const crewspaceSidebar = normalizePortableSidebarOrder(crewspaceExtension.sidebar);
  const crewspaceAgents = isPlainRecord(crewspaceExtension.agents) ? crewspaceExtension.agents : {};
  const crewspaceProjects = isPlainRecord(crewspaceExtension.projects) ? crewspaceExtension.projects : {};
  const crewspaceTasks = isPlainRecord(crewspaceExtension.tasks) ? crewspaceExtension.tasks : {};
  const crewspaceRoutines = isPlainRecord(crewspaceExtension.routines) ? crewspaceExtension.routines : {};
  const companyName =
  ...
  const manifest: CompanyPortabilityManifest = {
    ...
    company: {
      ...
      brandColor: asString(crewspaceCompany.brandColor),
      logoPath: asString(crewspaceCompany.logoPath) ?? asString(crewspaceCompany.logo),
      requireBoardApprovalForNewAgents:
        typeof crewspaceCompany.requireBoardApprovalForNewAgents === "boolean"
          ? crewspaceCompany.requireBoardApprovalForNewAgents
          : readCompanyApprovalDefault(companyFrontmatter),
    },
    sidebar: crewspaceSidebar,
    agents: [],
    skills: [],
    projects: [],
    issues: [],
  ...
  for (const agentPath of agentPaths) {
    ...
    const extension = isPlainRecord(crewspaceAgents[slug]) ? crewspaceAgents[slug] : {};
  ...
  for (const projectPath of projectPaths) {
    ...
    const slug = asString(frontmatter.slug) ?? fallbackSlug;
    const extension = isPlainRecord(crewspaceProjects[slug]) ? crewspaceProjects[slug] : {};
  ...
  for (const taskPath of taskPaths) {
    ...
    const slug = asString(frontmatter.slug) ?? fallbackSlug;
    const extension = isPlainRecord(crewspaceTasks[slug]) ? crewspaceTasks[slug] : {};
    const routineExtension = normalizeRoutineExtension(crewspaceRoutines[slug]);
    const routineExtensionRaw = isPlainRecord(crewspaceRoutines[slug]) ? crewspaceRoutines[slug] : {};
...
export function companyPortabilityService(db: Db, storage?: StorageService) {
    ...
    const candidatePaths = (tree.tree ?? [])
      ...
      .filter((entry) => {
        ...
        return (
          ...
          relative === ".crewspace.yaml" ||
          relative === ".crewspace.yml"
        );
      });
  ...
  async function exportBundle(
    ...
    const crewspaceAgentsOut: Record<string, Record<string, unknown>> = {};
    const crewspaceProjectsOut: Record<string, Record<string, unknown>> = {};
    const crewspaceTasksOut: Record<string, Record<string, unknown>> = {};
    const unportableTaskWorkspaceRefs = new Map<string, { workspaceId: string; taskSlugs: string[] }>();
    const crewspaceRoutinesOut: Record<string, Record<string, unknown>> = {};
    ...
    if (include.agents) {
      for (const agent of agentRows) {
        ...
        const desiredSkills = readCrewSpaceSkillSyncPreference(
        ...
        crewspaceAgentsOut[slug] = isPlainRecord(extension) ? extension : {};
      }
    }
    ...
    for (const project of selectedProjectRows) {
      ...
      crewspaceProjectsOut[slug] = isPlainRecord(extension) ? extension : {};
    }
    ...
    for (const issue of selectedIssueRows) {
      ...
      crewspaceTasksOut[taskSlug] = isPlainRecord(extension) ? extension : {};
    }
    ...
    for (const routine of selectedRoutineRows) {
      ...
      crewspaceRoutinesOut[taskSlug] = isPlainRecord(extension) ? extension : {};
    }

    const crewspaceExtensionPath = ".crewspace.yaml";
    const crewspaceAgents = Object.fromEntries(
      Object.entries(crewspaceAgentsOut).filter(([, value]) => isPlainRecord(value) && Object.keys(value).length > 0),
    );
    const crewspaceProjects = Object.fromEntries(
      Object.entries(crewspaceProjectsOut).filter(([, value]) => isPlainRecord(value) && Object.keys(value).length > 0),
    );
    const crewspaceTasks = Object.fromEntries(
      Object.entries(crewspaceTasksOut).filter(([, value]) => isPlainRecord(value) && Object.keys(value).length > 0),
    );
    const crewspaceRoutines = Object.fromEntries(
      Object.entries(crewspaceRoutinesOut).filter(([, value]) => isPlainRecord(value) && Object.keys(value).length > 0),
    );
    files[crewspaceExtensionPath] = buildYamlFile(
      {
        schema: "crewspace/v1",
        ...
        agents: Object.keys(crewspaceAgents).length > 0 ? crewspaceAgents : undefined,
        projects: Object.keys(crewspaceProjects).length > 0 ? crewspaceProjects : undefined,
        tasks: Object.keys(crewspaceTasks).length > 0 ? crewspaceTasks : undefined,
        routines: Object.keys(crewspaceRoutines).length > 0 ? crewspaceRoutines : undefined,
      },
      { preserveEmptyStrings: true },
    );

    let finalFiles = filterExportFiles(files, input.selectedFiles, crewspaceExtensionPath);
  ...
  async function importBundle(
    ...
    if (include.agents) {
      for (const planAgent of plan.preview.plan.agentPlans) {
        ...
        const adapterConfigWithSkills = writeCrewSpaceSkillSyncPreference(
          baseAdapterConfig,
          desiredSkills,
        );
