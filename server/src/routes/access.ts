...
function readSkillMarkdown(skillName: string): string | null {
  ...
  if (
    normalized !== "crewspace" &&
    normalized !== "crewspace-create-agent" &&
    normalized !== "crewspace-create-plugin" &&
...
/** Resolve the CrewSpace repo skills directory (built-in / managed skills). */
function resolveCrewSpaceSkillsDir(): string | null {
  const moduleDir = path.dirname(fileURLToPath(import.meta.url));
  const candidates = [
...
function listAvailableSkills(): AvailableSkill[] {
  ...
  const crewspaceSkillsDir = resolveCrewSpaceSkillsDir();

  // Build set of CrewSpace-managed skill names
  const crewspaceSkillNames = new Set<string>();
  if (crewspaceSkillsDir) {
    try {
      for (const entry of fs.readdirSync(crewspaceSkillsDir, { withFileTypes: true })) {
        if (entry.isDirectory()) crewspaceSkillNames.add(entry.name);
      }
    } catch { /* skip */ }
  }
  ...
  try {
    ...
    for (const entry of entries) {
      ...
      skills.push({
        name: entry.name,
        description,
        isCrewSpaceManaged: crewspaceSkillNames.has(entry.name),
      });
    }
...
export function buildJoinDefaultsPayloadForAccept(input: {
  ...
  if (!nonEmptyTrimmedString(merged.crewspaceApiUrl)) {
    const legacyCrewSpaceApiUrl = nonEmptyTrimmedString(input.crewspaceApiUrl);
    if (legacyCrewSpaceApiUrl) merged.crewspaceApiUrl = legacyCrewSpaceApiUrl;
  }
...
function summarizeOpenClawGatewayDefaultsForLog(defaultsPayload: unknown) {
  ...
  return {
    ...
    crewspaceApiUrl: defaults
      ? nonEmptyTrimmedString(defaults.crewspaceApiUrl)
      : null,
...
export function normalizeAgentDefaultsForJoin(input: {
  ...
  const rawCrewSpaceApiUrl =
    typeof defaults.crewspaceApiUrl === "string"
      ? defaults.crewspaceApiUrl.trim()
      : "";
  if (rawCrewSpaceApiUrl) {
    try {
      const parsedCrewSpaceApiUrl = new URL(rawCrewSpaceApiUrl);
      if (
        parsedCrewSpaceApiUrl.protocol !== "http:" &&
        parsedCrewSpaceApiUrl.protocol !== "https:"
      ) {
        diagnostics.push({
          code: "openclaw_gateway_crewspace_api_url_protocol",
          level: "warn",
          message: `crewspaceApiUrl must use http:// or https:// (got ${parsedCrewSpaceApiUrl.protocol}).`
        });
      } else {
        normalized.crewspaceApiUrl = parsedCrewSpaceApiUrl.toString();
        diagnostics.push({
          code: "openclaw_gateway_crewspace_api_url_configured",
          level: "info",
          message: `crewspaceApiUrl set to ${parsedCrewSpaceApiUrl.toString()}`
        });
      }
    } catch {
      diagnostics.push({
        code: "openclaw_gateway_crewspace_api_url_invalid",
        level: "warn",
        message: `Invalid crewspaceApiUrl: ${rawCrewSpaceApiUrl}`
      });
    }
  }
...
function buildOnboardingDiscoveryDiagnostics(input: {
  ...
  if (apiHost && isLoopbackHost(apiHost)) {
    diagnostics.push({
      ...
      level: "warn",
      message:
        "Onboarding URL resolves to loopback hostname. Remote OpenClaw agents cannot reach localhost on your CrewSpace host.",
  ...
  if (
    ...
    diagnostics.push({
      ...
      level: "warn",
      message: "CrewSpace is bound to loopback in authenticated/private mode.",
  ...
  if (
    ...
    diagnostics.push({
      ...
      hint: `Run pnpm crewspaceai allowed-hostname ${apiHost}`
    });
  }
...
function buildInviteOnboardingManifest(
  ...
  const baseUrl = requestBaseUrl(req);
  const skillPath = "/api/skills/crewspace";
  ...
  return {
    ...
    onboarding: {
      instructions:
        "Join as an OpenClaw Gateway agent, save your one-time claim secret, wait for board approval, then claim your API key. Save the claim response token to ~/.openclaw/workspace/crewspace-claimed-api-key.json and load CREWSPACE_API_KEY from that file before starting heartbeat loops. You MUST submit adapterType='openclaw_gateway', set agentDefaultsPayload.url to your ws:// or wss:// OpenClaw gateway endpoint, and include agentDefaultsPayload.headers.x-openclaw-token (or legacy x-openclaw-auth).",
      ...
      requiredFields: {
        ...
        agentDefaultsPayload:
          "Adapter config for OpenClaw gateway. MUST include url (ws:// or wss://) and headers.x-openclaw-token (or legacy x-openclaw-auth). Optional fields: crewspaceApiUrl, waitTimeoutMs, sessionKeyStrategy, sessionKey, role, scopes, disableDeviceAuth, devicePrivateKeyPem."
      },
      ...
      connectivity: {
        ...
        guidance:
          ...
          opts.deploymentExposure === "private"
            ? "If OpenClaw runs on another machine, ensure the CrewSpace hostname is reachable and allowed via `pnpm crewspaceai allowed-hostname <host>`."
            : "Ensure OpenClaw can reach this CrewSpace API base URL for invite, claim, and skill bootstrap calls."
      },
      textInstructions: {
      ...
      skill: {
        ...
        path: skillPath,
        url: skillUrl,
        installPath: "~/.openclaw/skills/crewspace/SKILL.md"
      }
    }
  };
}
...
export function buildInviteOnboardingTextDocument(
  ...
  appendBlock(`
    # CrewSpace OpenClaw Gateway Onboarding
  ...
  appendBlock(`
    ...
    - Keep device auth enabled (recommended). If devicePrivateKeyPem is omitted, CrewSpace generates and persists one during join so pairing approvals are stable.
    ...
    Body (JSON):
    {
      ...
      "agentDefaultsPayload": {
        ...
        "crewspaceApiUrl": "https://crewspace-hostname-your-agent-can-reach:3100",
    ...
    The board approves the join request in CrewSpace before key claim is allowed.
    ...
    - ~/.openclaw/workspace/crewspace-claimed-api-key.json
    chmod 600 ~/.openclaw/workspace/crewspace-claimed-api-key.json

    And set the CREWSPACE_API_KEY and CREWSPACE_API_URL in your environment variables as specified here:
    ...
    Then set CREWSPACE_API_KEY and CREWSPACE_API_URL from the saved token field for every heartbeat run.
    ...
    ## Step 4: Install CrewSpace skill in OpenClaw
    GET ${onboarding.skill.url}
    Install path: ${onboarding.skill.installPath}

    Be sure to prepend your CREWSPACE_API_URL to the top of your skill and note the path to your CREWSPACE_API_URL
    ...
    ${
      ...
      "Ensure CrewSpace is reachable from your OpenClaw runtime."
    }
  `);
  ...
  if (connectionCandidates.length > 0) {
    lines.push("## Suggested CrewSpace base URLs to try");
    ...
    appendBlock(`
      ...
      - set the first reachable candidate as agentDefaultsPayload.crewspaceApiUrl when submitting your join request

      If none are reachable: ask your human operator for a reachable hostname/address and help them update network configuration.
      For authenticated/private mode, they may need:
      - pnpm crewspaceai allowed-hostname <host>
      - then restart CrewSpace and retry onboarding.
    `);
  }
...
async function resolveActorEmail(db: Db, req: Request): Promise<string | null> {
  if (isLocalImplicit(req)) return "local@crewspace.local";
...
export function accessRoutes(
  ...
  router.get("/skills/index", (_req, res) => {
    res.json({
      skills: [
        { name: "crewspace", path: "/api/skills/crewspace" },
        {
        ...
        },
        {
          ...
          path: "/api/skills/crewspace-create-agent"
        }
      ]
    });
  });
  ...
  router.post(
    ...
    async (req, res) => {
      ...
      const gatewayDefaultsPayload =
        requestType === "agent"
          ? buildJoinDefaultsPayloadForAccept({
              ...
              crewspaceApiUrl: req.body.crewspaceApiUrl ?? null,
      ...
      if (requestType === "agent" && adapterType === "openclaw_gateway") {
        ...
        if (
          expectedDefaults.crewspaceApiUrl &&
          !persistedDefaults.crewspaceApiUrl
        ) {
          missingPersistedFields.push("crewspaceApiUrl");
        }
