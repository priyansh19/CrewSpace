...
import type { CompanyPortabilityFileEntry } from "@crewspaceai/shared";
...
describe("company portability", () => {
    ...
    const companySkills = [
      {
        ...
        slug: "crewspace",
        name: "crewspace",
        description: "CrewSpace coordination skill",
        markdown: "---\nname: crewspace\ndescription: CrewSpace coordination skill\n---\n\n# CrewSpace\n",
        sourceType: "github",
        sourceLocator: "https://github.com/crewspaceai/crewspace/tree/master/skills/crewspace",
  ...
  it("parses canonical GitHub import URLs with explicit ref and package path", () => {
    expect(
      parseGitHubSourceUrl("https://github.com/crewspaceai/companies?ref=feature%2Fdemo&path=gstack"),
    ).toEqual({
      owner: "crewspaceai",
      repo: "companies",
      ref: "feature/demo",
  ...
  it("parses canonical GitHub import URLs with explicit companyPath", () => {
    expect(
      parseGitHubSourceUrl(
        "https://github.com/crewspaceai/companies?ref=abc123&companyPath=gstack%2FCOMPANY.md",
      ),
    ).toEqual({
      owner: "crewspaceai",
      repo: "companies",
  ...
  it("exports referenced skills as stubs by default with sanitized CrewSpace extension data", async () => {
    ...
    expect(asTextFile(exported.files["COMPANY.md"])).toContain('name: "CrewSpace"');
    ...
    expect(asTextFile(exported.files["agents/claudecoder/AGENTS.md"])).toContain(`- "${crewspaceKey}"`);
    expect(asTextFile(exported.files["agents/cmo/AGENTS.md"])).not.toContain("skills:");
    expect(asTextFile(exported.files["skills/crewspaceai/crewspace/crewspace/SKILL.md"])).toContain("metadata:");
    expect(asTextFile(exported.files["skills/crewspaceai/crewspace/crewspace/SKILL.md"])).toContain('kind: "github-dir"');
    expect(exported.files["skills/crewspaceai/crewspace/crewspace/references/api.md"]).toBeUndefined();
    ...
    const extension = asTextFile(exported.files[".crewspace.yaml"]);
    expect(extension).toContain('schema: "crewspace/v1"');
    ...
    expect(extension).not.toContain("crewspaceSkillSync");
  ...
  it("exports default sidebar order into the CrewSpace extension and manifest", async () => {
    const portability = companyPortabilityService({} as any);
    ...
    expect(asTextFile(exported.files[".crewspace.yaml"])).toContain([
      "sidebar:",
      "  agents:",
      '    - "claudecoder"',
      '    - "cmo"',
  ...
  it("expands referenced skills when requested", async () => {
    ...
    expect(asTextFile(exported.files["skills/crewspaceai/crewspace/crewspace/SKILL.md"])).toContain("# CrewSpace");
    expect(asTextFile(exported.files["skills/crewspaceai/crewspace/crewspace/SKILL.md"])).toContain("metadata:");
    expect(asTextFile(exported.files["skills/crewspaceai/crewspace/crewspace/references/api.md"])).toContain("# API");
  });
  ...
  it("exports only selected skills when skills filter is provided", async () => {
    ...
    expect(asTextFile(exported.files["skills/company/PAP/company-playbook/SKILL.md"])).toContain("# Company Playbook");
    expect(exported.files["skills/crewspaceai/crewspace/crewspace/SKILL.md"]).toBeUndefined();
  });
  ...
  it("warns and exports all skills when skills filter matches nothing", async () => {
    ...
    expect(exported.files["skills/company/PAP/company-playbook/SKILL.md"]).toBeDefined();
    expect(exported.files["skills/crewspaceai/crewspace/crewspace/SKILL.md"]).toBeDefined();
  });

  it("exports the company logo into images/ and references it from .crewspace.yaml", async () => {
    const storage = {
    ...
    expect(exported.files[".crewspace.yaml"]).toContain('logoPath: "images/company-logo.png"');
  });
  ...
  it("exports duplicate skill slugs into readable namespaced paths", async () => {
    ...
    companySkillSvc.listFull.mockResolvedValue([
      ...
      },
      {
        id: "skill-crewspace",
        companyId: "company-1",
        key: "crewspaceai/crewspace/release-changelog",
        slug: "release-changelog",
        name: "release-changelog",
        description: "Bundled release changelog skill",
        markdown: "---\nname: release-changelog\n---\n\n# Bundled Release Changelog\n",
        sourceType: "github",
        sourceLocator: "https://github.com/crewspaceai/crewspace/tree/master/skills/release-changelog",
        sourceRef: "0123456789abcdef0123456789abcdef01234567",
        trustLevel: "markdown_only",
        compatibility: "compatible",
        fileInventory: [{ path: "SKILL.md", kind: "skill" }],
        metadata: {
          sourceKind: "crewspace_bundled",
          owner: "crewspaceai",
          repo: "crewspace",
          ref: "0123456789abcdef0123456789abcdef01234567",
          trackingRef: "master",
    ...
    expect(asTextFile(exported.files["skills/crewspaceai/crewspace/release-changelog/SKILL.md"])).toContain("metadata:");
    expect(asTextFile(exported.files["skills/crewspaceai/crewspace/release-changelog/SKILL.md"])).toContain("crewspaceai/crewspace/release-changelog");
  });
  ...
  it("exports portable project workspace metadata and remaps it on import", async () => {
        ...
        workspaces: [
          {
            ...
            name: "Main Repo",
            sourceType: "git_repo",
            cwd: "/Users/dotta/crewspace",
            repoUrl: "https://github.com/crewspaceai/crewspace.git",
            repoRef: "main",
            defaultRef: "main",
            visibility: "default",
            setupCommand: "pnpm install",
            cleanupCommand: "rm -rf .crewspace-tmp",
            remoteProvider: null,
    ...
    expect(extension).toContain('repoUrl: "https://github.com/crewspaceai/crewspace.git"');
    expect(extension).toContain('defaultProjectWorkspaceKey: "main-repo"');
    expect(extension).toContain('projectWorkspaceKey: "main-repo"');
    expect(extension).not.toContain("/Users/dotta/crewspace");
    ...
    await portability.importBundle({
      ...
      target: {
        mode: "new_company",
        newCompanyName: "Imported CrewSpace",
      },
  ...
  it("infers portable git metadata from a local checkout without task warning fan-out", async () => {
    const portability = companyPortabilityService({} as any);
    const repoDir = await fs.mkdtemp(path.join(os.tmpdir(), "crewspace-portability-git-"));
    execFileSync("git", ["init"], { cwd: repoDir, stdio: "ignore" });
    execFileSync("git", ["checkout", "-b", "main"], { cwd: repoDir, stdio: "ignore" });
    execFileSync("git", ["remote", "add", "origin", "https://github.com/crewspaceai/crewspace.git"], {
      cwd: repoDir,
      stdio: "ignore",
    });
    ...
    const extension = asTextFile(exported.files[".crewspace.yaml"]);
    expect(extension).toContain('repoUrl: "https://github.com/crewspaceai/crewspace.git"');
    expect(extension).toContain('projectWorkspaceKey: "crewspace"');
  ...
  it("reads env inputs back from .crewspace.yaml during preview import", async () => {
    const portability = companyPortabilityService({} as any);
  ...
  it("exports routines as recurring task packages with CrewSpace routine extensions", async () => {
    const portability = companyPortabilityService({} as any);
  ...
  it("imports recurring task packages as routines instead of one-time issues", async () => {
    ...
    const preview = await portability.previewImport({
      source: { type: "inline", rootPath: "crewspace-demo", files },
      include: { company: true, agents: true, projects: true, issues: true, skills: false },
      target: { mode: "new_company", newCompanyName: "Imported CrewSpace" },
      agents: "all",
      collisionStrategy: "rename",
    });
  ...
  it("migrates legacy schedule.recurrence imports into routine triggers", async () => {
    ...
    const files = {
      "COMPANY.md": ['---', 'schema: "agentcompanies/v1"', 'name: "Imported CrewSpace"', "---", ""].join("\n"),
  ...
  it("imports a vendor-neutral package without .crewspace.yaml", async () => {
    const portability = companyPortabilityService({} as any);
    ...
    expect(preview.errors).toEqual([]);
    expect(preview.manifest.company?.name).toBe("Imported CrewSpace");
  ...
  it("imports a packaged company logo and attaches it to the target company", async () => {
    ...
    exported.files[".crewspace.yaml"] = `${exported.files[".crewspace.yaml"]}`.replace(
      'brandColor: "#5c5fff"\n',
