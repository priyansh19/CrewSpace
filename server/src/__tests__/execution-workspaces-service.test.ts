...
async function createTempRepo() {
  const repoRoot = await fs.mkdtemp(path.join(os.tmpdir(), "crewspace-execution-workspace-"));
  await runGit(repoRoot, ["init"]);
  await runGit(repoRoot, ["config", "user.name", "CrewSpace Test"]);
  await runGit(repoRoot, ["config", "user.email", "test@crewspace.local"]);
...
describeEmbeddedPostgres("executionWorkspaceService.getCloseReadiness", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("crewspace-execution-workspaces-service-");
  ...
  it("warns about dirty and unmerged git worktrees and reports cleanup actions", async () => {
    ...
    const worktreePath = path.join(path.dirname(repoRoot), `crewspace-worktree-${randomUUID()}`);
    tempDirs.add(worktreePath);

    await runGit(repoRoot, ["branch", "crewspace-close-check"]);
    await runGit(repoRoot, ["worktree", "add", worktreePath, "crewspace-close-check"]);
    ...
    await db.insert(executionWorkspaces).values({
      ...
      branchName: "crewspace-close-check",
      baseRef: "main",
      metadata: {
