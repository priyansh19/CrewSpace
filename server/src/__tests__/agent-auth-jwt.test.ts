...
describe("agent local JWT", () => {
  const secretEnv = "CREWSPACE_AGENT_JWT_SECRET";
  const ttlEnv = "CREWSPACE_AGENT_JWT_TTL_SECONDS";
  const issuerEnv = "CREWSPACE_AGENT_JWT_ISSUER";
  const audienceEnv = "CREWSPACE_AGENT_JWT_AUDIENCE";
  ...
  it("rejects issuer/audience mismatch", () => {
    ...
    process.env[issuerEnv] = "crewspace";
    process.env[audienceEnv] = "crewspace-api";
    expect(verifyLocalAgentJwt(token!)).toBeNull();
  });
});
