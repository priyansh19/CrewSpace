...
describe("worktree config repair", () => {
  it("repairs legacy repo-local worktree config and env files into an isolated instance", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "crewspace-worktree-repair-"));
    const worktreeRoot = path.join(tempRoot, "PAP-884-ai-commits-component");
    const crewspaceDir = path.join(worktreeRoot, ".crewspace");
    const configPath = path.join(crewspaceDir, "config.json");
    const envPath = path.join(crewspaceDir, ".env");
    const sharedRoot = path.join(tempRoot, ".crewspace", "instances", "default");
    const isolatedHome = path.join(tempRoot, ".crewspace-worktrees");

    await fs.mkdir(crewspaceDir, { recursive: true });
    await fs.writeFile(configPath, JSON.stringify(buildLegacyConfig(sharedRoot), null, 2) + "\n", "utf8");
    await fs.writeFile(
      envPath,
      [
        "# CrewSpace environment variables",
        "CREWSPACE_IN_WORKTREE=true",
        "CREWSPACE_WORKTREE_NAME=PAP-884-ai-commits-component",
        "CREWSPACE_AGENT_JWT_SECRET=shared-secret",
        "",
      ].join("\n"),
      "utf8",
    );
    ...
    process.env.CREWSPACE_WORKTREE_NAME = "PAP-884-ai-commits-component";
    process.env.CREWSPACE_WORKTREES_DIR = isolatedHome;
    ...
    expect(repairedEnv).toContain(`CREWSPACE_HOME=${JSON.stringify(isolatedHome)}`);
    expect(repairedEnv).toContain('CREWSPACE_INSTANCE_ID="pap-884-ai-commits-component"');
    expect(repairedEnv).toContain(`CREWSPACE_CONFIG=${JSON.stringify(await fs.realpath(configPath))}`);
    expect(repairedEnv).toContain(`CREWSPACE_CONTEXT=${JSON.stringify(path.join(isolatedHome, "context.json"))}`);
    expect(repairedEnv).toContain('CREWSPACE_AGENT_JWT_SECRET="shared-secret"');
    expect(process.env.CREWSPACE_HOME).toBe(isolatedHome);
    expect(process.env.CREWSPACE_INSTANCE_ID).toBe("pap-884-ai-commits-component");
  });

  it("avoids sibling worktree ports when repairing legacy configs", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "crewspace-worktree-repair-ports-"));
    ...
    await fs.writeFile(
      ...
      [
        "# CrewSpace environment variables",
        "CREWSPACE_IN_WORKTREE=true",
        "CREWSPACE_WORKTREE_NAME=PAP-880-thumbs-capture-for-evals-feature",
        "",
      ].join("\n"),
      "utf8",
    );
    ...
    process.chdir(worktreeRoot);
    process.env.CREWSPACE_IN_WORKTREE = "true";
    process.env.CREWSPACE_WORKTREE_NAME = "PAP-880-thumbs-capture-for-evals-feature";
    process.env.CREWSPACE_WORKTREES_DIR = isolatedHome;
  ...
  it("rebalances duplicate ports for already isolated worktree configs", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "crewspace-worktree-rebalance-"));
    const isolatedHome = path.join(tempRoot, ".crewspace-worktrees");
    const repoWorktreesRoot = path.join(tempRoot, "repo", ".crewspace", "worktrees");
    ...
    const crewspaceDir = path.join(currentWorktreeRoot, ".crewspace");
    const configPath = path.join(crewspaceDir, "config.json");
    const envPath = path.join(crewspaceDir, ".env");
    const currentInstanceRoot = path.join(isolatedHome, "instances", "pap-884-ai-commits-component");
    const siblingConfigPath = path.join(siblingWorktreeRoot, ".crewspace", "config.json");
  ...
  it("persists runtime-selected worktree ports back into config", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "crewspace-worktree-ports-"));
    ...
    process.chdir(worktreeRoot);
    process.env.CREWSPACE_IN_WORKTREE = "true";
    process.env.CREWSPACE_WORKTREE_NAME = "PAP-878-create-a-mine-tab-in-inbox";
    process.env.CREWSPACE_HOME = isolatedHome;
    process.env.CREWSPACE_INSTANCE_ID = "pap-878-create-a-mine-tab-in-inbox";
