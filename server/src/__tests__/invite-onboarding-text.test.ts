...
describe("buildInviteOnboardingTextDocument", () => {
  it("renders a plain-text onboarding doc with expected endpoint references", () => {
    ...
    expect(text).toContain("CrewSpace OpenClaw Gateway Onboarding");
    ...
    expect(text).toContain("Suggested CrewSpace base URLs to try");
    expect(text).toContain("http://localhost:3100");
    expect(text).toContain("host.docker.internal");
    expect(text).toContain("crewspaceApiUrl");
    ...
    expect(text).toContain("set the first reachable candidate as agentDefaultsPayload.crewspaceApiUrl");
    expect(text).toContain("~/.openclaw/workspace/crewspace-claimed-api-key.json");
    expect(text).toContain("CREWSPACE_API_KEY");
