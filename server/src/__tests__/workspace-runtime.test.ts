...
import { resolveCrewSpaceConfigPath } from "../paths.ts";
import type { WorkspaceOperation } from "@crewspaceai/shared";
...
async function createTempRepo() {
  const repoRoot = await fs.mkdtemp(path.join(os.tmpdir(), "crewspace-worktree-repo-"));
...
afterEach(async () => {
  ...
  delete process.env.CREWSPACE_WORKTREES_DIR;
  delete process.env.DATABASE_URL;
  await resetRuntimeServicesForTests();
});

describe("sanitizeRuntimeServiceBaseEnv", () => {
  it("removes inherited CrewSpace and pnpm auth flags before spawning runtime services", () => {
    const sanitized = sanitizeRuntimeServiceBaseEnv({
      PATH: process.env.PATH,
      DATABASE_URL: "postgres://example.test/crewspace",
      CREWSPACE_HOME: "/tmp/crewspace-home",
      CREWSPACE_INSTANCE_ID: "runtime-instance",
      npm_config_tailscale_auth: "true",
      npm_config_authenticated_private: "true",
      HOST: "0.0.0.0",
    });

    expect(sanitized.CREWSPACE_HOME).toBeUndefined();
    expect(sanitized.CREWSPACE_INSTANCE_ID).toBeUndefined();
...
describe("realizeExecutionWorkspace", () => {
  it("creates and reuses a git worktree for an issue-scoped branch", async () => {
    ...
    expect(first.cwd).toContain(path.join(".crewspace", "worktrees"));
  ...
  it("runs a configured provision command inside the derived worktree", async () => {
    ...
    await fs.writeFile(
      ...
      [
        "#!/usr/bin/env bash",
        "set -euo pipefail",
        "printf '%s\\n' \"$CREWSPACE_WORKSPACE_BRANCH\" > .crewspace-provision-branch",
        "printf '%s\\n' \"$CREWSPACE_WORKSPACE_BASE_CWD\" > .crewspace-provision-base",
        "printf '%s\\n' \"$CREWSPACE_WORKSPACE_CREATED\" > .crewspace-provision-created",
      ].join("\n"),
      "utf8",
    );
    ...
    await expect(fs.readFile(path.join(workspace.cwd, ".crewspace-provision-branch"), "utf8")).resolves.toBe(
      "PAP-448-run-provision-command\n",
    );
    await expect(fs.readFile(path.join(workspace.cwd, ".crewspace-provision-base"), "utf8")).resolves.toBe(
      `${repoRoot}\n`,
    );
    await expect(fs.readFile(path.join(workspace.cwd, ".crewspace-provision-created"), "utf8")).resolves.toBe(
      "true\n",
    );
    ...
    await expect(fs.readFile(path.join(reused.cwd, ".crewspace-provision-created"), "utf8")).resolves.toBe("false\n");
  });

  it("writes an isolated repo-local CrewSpace config and worktree branding when provisioning", async () => {
    const repoRoot = await createTempRepo();
    const previousCwd = process.cwd();
    const crewspaceHome = await fs.mkdtemp(path.join(os.tmpdir(), "crewspace-worktree-home-"));
    const isolatedWorktreeHome = await fs.mkdtemp(path.join(os.tmpdir(), "crewspace-worktrees-"));
    const instanceId = "worktree-base";
    const sharedConfigDir = path.join(crewspaceHome, "instances", instanceId);
    const sharedConfigPath = path.join(sharedConfigDir, "config.json");
    const sharedEnvPath = path.join(sharedConfigDir, ".env");

    process.env.CREWSPACE_HOME = crewspaceHome;
    process.env.CREWSPACE_INSTANCE_ID = instanceId;
    process.env.CREWSPACE_WORKTREES_DIR = isolatedWorktreeHome;
    ...
    await fs.writeFile(sharedEnvPath, 'DATABASE_URL="postgres://worktree:test@db.example.com:6543/crewspace"\n', "utf8");
    ...
    try {
      ...
      const configPath = path.join(workspace.cwd, ".crewspace", "config.json");
      const envPath = path.join(workspace.cwd, ".crewspace", ".env");
      ...
      expect(envContents).toContain(`CREWSPACE_HOME=${JSON.stringify(isolatedWorktreeHome)}`);
      expect(envContents).toContain(`CREWSPACE_INSTANCE_ID=${JSON.stringify(expectedInstanceId)}`);
      expect(envContents).toContain(`CREWSPACE_CONFIG=${JSON.stringify(configPath)}`);
      expect(envContents).toContain("CREWSPACE_IN_WORKTREE=true");
      expect(envContents).toContain(
        `CREWSPACE_WORKTREE_NAME=${JSON.stringify("PAP-885-show-worktree-banner")}`,
      );

      process.chdir(workspace.cwd);
      expect(resolveCrewSpaceConfigPath()).toBe(configPath);
    } finally {
  ...
  it("auto-detects the default branch when baseRef is not configured", async () => {
    ...
    const repoRoot = await fs.mkdtemp(path.join(os.tmpdir(), "crewspace-worktree-master-"));
    ...
    const bareRemote = await fs.mkdtemp(path.join(os.tmpdir(), "crewspace-worktree-bare-"));
  ...
  it("auto-detects the default branch via symbolic-ref when origin/HEAD is set", async () => {
    ...
    const repoRoot = await fs.mkdtemp(path.join(os.tmpdir(), "crewspace-worktree-symref-"));
    await runGit(repoRoot, ["init", "-b", "master"]);
    await runGit(repoRoot, ["config", "user.email", "crewspace@example.com"]);
    await runGit(repoRoot, ["config", "user.name", "CrewSpace Test"]);
    await fs.writeFile(path.join(repoRoot, "README.md"), "hello\n", "utf8");
    ...
    const bareRemote = await fs.mkdtemp(path.join(os.tmpdir(), "crewspace-worktree-bare-symref-"));
    await runGit(bareRemote, ["init", "--bare"]);
    await runGit(repoRoot, ["remote", "add", "origin", bareRemote]);
    await runGit(repoRoot, ["push", "-u", "origin", "master"]);
    await runGit(repoRoot, ["fetch", "origin"]);
...
describe("ensureRuntimeServicesForRun", () => {
  it("reuses shared runtime services across runs and starts a new service after release", async () => {
    const workspaceRoot = await fs.mkdtemp(path.join(os.tmpdir(), "crewspace-runtime-workspace-"));
  ...
  it("does not reuse project-scoped shared services across different workspace launch contexts", async () => {
    const primaryWorkspaceRoot = await fs.mkdtemp(path.join(os.tmpdir(), "crewspace-runtime-primary-"));
    const worktreeWorkspaceRoot = path.join(primaryWorkspaceRoot, ".crewspace", "worktrees", "PAP-874-chat-speed-issues");
    ...
    const serviceCommand =
      "node -e \"require('node:http').createServer((req,res)=>res.end(process.env.CREWSPACE_HOME)).listen(Number(process.env.PORT), '127.0.0.1')\"";
    const config = {
      workspaceRuntime: {
        services: [
          {
            ...
            cwd: ".",
            env: {
              CREWSPACE_HOME: "{{workspace.cwd}}/.crewspace/runtime-services",
            },
            port: { type: "auto" },
            readiness: {
              type: "http",
    ...
    expect(await primaryResponse.text()).toBe(path.join(primaryWorkspaceRoot, ".crewspace", "runtime-services"));

    const executionResponse = await fetch(executionServices[0]!.url!);
    expect(await executionResponse.text()).toBe(path.join(worktreeWorkspaceRoot, ".crewspace", "runtime-services"));
  });

  it("does not leak parent CrewSpace instance env into runtime service commands", async () => {
    const workspaceRoot = await fs.mkdtemp(path.join(os.tmpdir(), "crewspace-runtime-env-"));
    const workspace = buildWorkspace(workspaceRoot);
    ...
    const serviceCommand = [
      ...
      JSON.stringify(
        [
          ...
          "crewspaceConfig: process.env.CREWSPACE_CONFIG ?? null,",
          "crewspaceHome: process.env.CREWSPACE_HOME ?? null,",
          "crewspaceInstanceId: process.env.CREWSPACE_INSTANCE_ID ?? null,",
    ...
    process.env.CREWSPACE_CONFIG = "/tmp/base-crewspace-config.json";
    process.env.CREWSPACE_HOME = "/tmp/base-crewspace-home";
    process.env.CREWSPACE_INSTANCE_ID = "base-instance";
    process.env.DATABASE_URL = "postgres://shared-db.example.com/crewspace";
    ...
    expect(captured.crewspaceConfig).toBeNull();
    expect(captured.crewspaceHome).toBeNull();
    expect(captured.crewspaceInstanceId).toBeNull();
  ...
  it("stops execution workspace runtime services by executionWorkspaceId", async () => {
    const workspaceRoot = await fs.mkdtemp(path.join(os.tmpdir(), "crewspace-runtime-stop-"));
    const workspace = buildWorkspace(workspaceRoot);
  ...
  it("does not stop services in sibling directories when matching by workspace cwd", async () => {
    const workspaceParent = await fs.mkdtemp(path.join(os.tmpdir(), "crewspace-runtime-sibling-"));
...
describeEmbeddedPostgres("workspace runtime startup reconciliation", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("crewspace-workspace-runtime-");
  ...
  it("adopts a live auto-port shared service after runtime state is reset", async () => {
    const workspaceRoot = await fs.mkdtemp(path.join(os.tmpdir(), "crewspace-runtime-reconcile-"));
    const crewspaceHome = await fs.mkdtemp(path.join(os.tmpdir(), "crewspace-runtime-home-"));
    process.env.CREWSPACE_HOME = crewspaceHome;
    process.env.CREWSPACE_INSTANCE_ID = `runtime-reconcile-${randomUUID()}`;
  ...
  it("persists controlled execution workspace stops as stopped", async () => {
    const workspaceRoot = await fs.mkdtemp(path.join(os.tmpdir(), "crewspace-runtime-stop-persisted-"));
    const companyId = randomUUID();
    const agentId = randomUUID();
