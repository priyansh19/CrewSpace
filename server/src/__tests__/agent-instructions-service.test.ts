...
describe("agent instructions service", () => {
  const originalCrewSpaceHome = process.env.CREWSPACE_HOME;
  const originalCrewSpaceInstanceId = process.env.CREWSPACE_INSTANCE_ID;
  const cleanupDirs = new Set<string>();

  afterEach(async () => {
    if (originalCrewSpaceHome === undefined) delete process.env.CREWSPACE_HOME;
    else process.env.CREWSPACE_HOME = originalCrewSpaceHome;
    if (originalCrewSpaceInstanceId === undefined) delete process.env.CREWSPACE_INSTANCE_ID;
    else process.env.CREWSPACE_INSTANCE_ID = originalCrewSpaceInstanceId;
  ...
  it("copies the existing bundle into the managed root when switching to managed mode", async () => {
    const crewspaceHome = await makeTempDir("crewspace-agent-instructions-home-");
    const externalRoot = await makeTempDir("crewspace-agent-instructions-external-");
    cleanupDirs.add(crewspaceHome);
    cleanupDirs.add(externalRoot);
    process.env.CREWSPACE_HOME = crewspaceHome;
    process.env.CREWSPACE_INSTANCE_ID = "test-instance";
  ...
  it("creates the target entry file when switching to a new external root", async () => {
    ...
    const externalRoot = await makeTempDir("crewspace-agent-instructions-new-external-");
    cleanupDirs.add(crewspaceHome);
    cleanupDirs.add(externalRoot);
    process.env.CREWSPACE_HOME = crewspaceHome;
    process.env.CREWSPACE_INSTANCE_ID = "test-instance";
  ...
  it("filters junk files, dependency bundles, and python caches from bundle listings and exports", async () => {
    const externalRoot = await makeTempDir("crewspace-agent-instructions-ignore-");
    cleanupDirs.add(externalRoot);
  ...
  it("recovers a managed bundle from disk when bundle config metadata is missing", async () => {
    const crewspaceHome = await makeTempDir("crewspace-agent-instructions-recover-");
    cleanupDirs.add(crewspaceHome);
    process.env.CREWSPACE_HOME = crewspaceHome;
    process.env.CREWSPACE_INSTANCE_ID = "test-instance";
  ...
  it("prefers the managed bundle on disk when managed metadata points at a stale root", async () => {
    const crewspaceHome = await makeTempDir("crewspace-agent-instructions-stale-managed-");
    const staleRoot = await makeTempDir("crewspace-agent-instructions-stale-root-");
    cleanupDirs.add(crewspaceHome);
  ...
  it("heals stale managed metadata when writing bundle files", async () => {
    const crewspaceHome = await makeTempDir("crewspace-agent-instructions-heal-write-");
    const staleRoot = await makeTempDir("crewspace-agent-instructions-heal-write-stale-");
    cleanupDirs.add(crewspaceHome);
    cleanupDirs.add(staleRoot);
    process.env.CREWSPACE_HOME = crewspaceHome;
    process.env.CREWSPACE_INSTANCE_ID = "test-instance";
  ...
  it("heals stale managed metadata when deleting bundle files", async () => {
    const crewspaceHome = await makeTempDir("crewspace-agent-instructions-heal-delete-");
    const staleRoot = await makeTempDir("crewspace-agent-instructions-heal-delete-stale-");
    cleanupDirs.add(crewspaceHome);
    cleanupDirs.add(staleRoot);
    process.env.CREWSPACE_HOME = crewspaceHome;
    process.env.CREWSPACE_INSTANCE_ID = "test-instance";
  ...
  it("recovers the managed bundle when stale root metadata is present but mode is missing", async () => {
    const crewspaceHome = await makeTempDir("crewspace-agent-instructions-partial-managed-");
    const staleRoot = await makeTempDir("crewspace-agent-instructions-partial-root-");
    cleanupDirs.add(crewspaceHome);
    cleanupDirs.add(staleRoot);
    process.env.CREWSPACE_HOME = crewspaceHome;
    process.env.CREWSPACE_INSTANCE_ID = "test-instance";
