const FAVICON_BLOCK_START = "<!-- CREWSPACE_FAVICON_START -->";
const FAVICON_BLOCK_END = "<!-- CREWSPACE_FAVICON_END -->";
const RUNTIME_BRANDING_BLOCK_START = "<!-- CREWSPACE_RUNTIME_BRANDING_START -->";
const RUNTIME_BRANDING_BLOCK_END = "<!-- CREWSPACE_RUNTIME_BRANDING_END -->";
...
export function isWorktreeUiBrandingEnabled(env: NodeJS.ProcessEnv = process.env): boolean {
  return isTruthyEnvValue(env.CREWSPACE_IN_WORKTREE);
}
...
export function getWorktreeUiBranding(env: NodeJS.ProcessEnv = process.env): WorktreeUiBranding {
  ...
  const name = nonEmpty(env.CREWSPACE_WORKTREE_NAME) ?? nonEmpty(env.CREWSPACE_INSTANCE_ID) ?? "worktree";
  const color = normalizeHexColor(env.CREWSPACE_WORKTREE_COLOR) ?? deriveColorFromSeed(name);
...
export function renderRuntimeBrandingMeta(branding: WorktreeUiBranding): string {
  ...
  return [
    '<meta name="crewspace-worktree-enabled" content="true" />',
    `<meta name="crewspace-worktree-name" content="${escapeHtmlAttribute(branding.name)}" />`,
    `<meta name="crewspace-worktree-color" content="${escapeHtmlAttribute(branding.color)}" />`,
    `<meta name="crewspace-worktree-text-color" content="${escapeHtmlAttribute(branding.textColor)}" />`,
  ].join("\n");
}
