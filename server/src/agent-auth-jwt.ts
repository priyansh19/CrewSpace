...
function jwtConfig() {
  const secret = process.env.CREWSPACE_AGENT_JWT_SECRET;
  if (!secret) return null;

  return {
    secret,
    ttlSeconds: parseNumber(process.env.CREWSPACE_AGENT_JWT_TTL_SECONDS, 60 * 60 * 48),
    issuer: process.env.CREWSPACE_AGENT_JWT_ISSUER ?? "crewspace",
    audience: process.env.CREWSPACE_AGENT_JWT_AUDIENCE ?? "crewspace-api",
  };
}
