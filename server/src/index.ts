...
export async function startServer(): Promise<StartedServer> {
  ...
  if (process.env.CREWSPACE_SECRETS_PROVIDER === undefined) {
    process.env.CREWSPACE_SECRETS_PROVIDER = config.secretsProvider;
  }
  if (process.env.CREWSPACE_SECRETS_STRICT_MODE === undefined) {
    process.env.CREWSPACE_SECRETS_STRICT_MODE = config.secretsStrictMode ? "true" : "false";
  }
  if (process.env.CREWSPACE_SECRETS_MASTER_KEY_FILE === undefined) {
    process.env.CREWSPACE_SECRETS_MASTER_KEY_FILE = config.secretsMasterKeyFilePath;
  }
  ...
  async function promptApplyMigrations(migrations: string[]): Promise<boolean> {
    if (process.env.CREWSPACE_MIGRATION_AUTO_APPLY === "true") return true;
    if (process.env.CREWSPACE_MIGRATION_PROMPT === "never") return false;
      ...
      if (!apply) {
        throw new Error(
          `${label} has pending migrations (${formatPendingMigrationSummary(state.pendingMigrations)}). ` +
            "Refusing to start against a stale schema. Run pnpm db:migrate or set CREWSPACE_MIGRATION_AUTO_APPLY=true.",
        );
      }
  ...
  const LOCAL_BOARD_USER_EMAIL = "local@crewspace.local";
  ...
  if (config.databaseUrl) {
    ...
    const verboseEmbeddedPostgresLogs = process.env.CREWSPACE_EMBEDDED_POSTGRES_VERBOSE === "true";
    ...
    if (!postgresActuallyRunning) {
      ...
      try {
        ...
        await ensurePostgresDatabase(configuredAdminConnectionString, "crewspace");
        logger.warn(
    ...
    const embeddedAdminConnectionString = `postgres://crewspace:crewspace@127.0.0.1:${port}/postgres`;
    const dbStatus = await ensurePostgresDatabase(embeddedAdminConnectionString, "crewspace");
    if (dbStatus === "created") {
      logger.info("Created embedded PostgreSQL database: crewspace");
    }
  ...
  if (config.deploymentMode === "authenticated") {
    ...
    const betterAuthSecret =
      process.env.BETTER_AUTH_SECRET?.trim() ?? process.env.CREWSPACE_AGENT_JWT_SECRET?.trim();
    if (!betterAuthSecret) {
      throw new Error(
        "authenticated mode requires BETTER_AUTH_SECRET (or CREWSPACE_AGENT_JWT_SECRET) to be set",
      );
    }
  ...
  const runtimeApiHost =
    runtimeListenHost === "0.0.0.0" || runtimeListenHost === "::"
      ? "localhost"
      : runtimeListenHost;
  process.env.CREWSPACE_LISTEN_HOST = runtimeListenHost;
  process.env.CREWSPACE_LISTEN_PORT = String(listenPort);
  process.env.CREWSPACE_API_URL = `http://${runtimeApiHost}:${listenPort}`;
  ...
  return {
    ...
    host: config.host,
    listenPort,
    apiUrl: process.env.CREWSPACE_API_URL ?? `http://${runtimeApiHost}:${listenPort}`,
...
if (isMainModule(import.meta.url)) {
  void startServer().catch((err) => {
    logger.error({ err }, "CrewSpace server failed to start");
    process.exit(1);
  });
}
