...
export function getWorktreeUiBranding(): WorktreeUiBranding | null {
  if (readMetaContent("crewspace-worktree-enabled") !== "true") return null;

  const name = readMetaContent("crewspace-worktree-name");
  const color = normalizeHexColor(readMetaContent("crewspace-worktree-color"));
  ...
  return {
    ...
    name,
    color,
    textColor: normalizeHexColor(readMetaContent("crewspace-worktree-text-color")) ?? pickReadableTextColor(color),
  };
}
