 ...
 *
 * Plugin bundles are built with `external: ["@crewspaceai/plugin-sdk/ui", "react", "react-dom"]`,
 * so their ESM output contains bare specifier imports like:
 *
 * ```js
 * import { usePluginData } from "@crewspaceai/plugin-sdk/ui";
 ...
 *    host's global bridge registry (`globalThis.__crewspacePluginBridge__`)
 ...
 * - `import { ... } from "@crewspaceai/plugin-sdk/ui";`
 *
...
function rewriteBareSpecifiers(source: string): string {
  ...
  const rewrites: Record<string, string> = {
    '"@crewspaceai/plugin-sdk/ui"': `"${getShimBlobUrl("sdk-ui")}"`,
    "'@crewspaceai/plugin-sdk/ui'": `'${getShimBlobUrl("sdk-ui")}'`,
    '"@crewspaceai/plugin-sdk/ui/hooks"': `"${getShimBlobUrl("sdk-ui")}"`,
    "'@crewspaceai/plugin-sdk/ui/hooks'": `'${getShimBlobUrl("sdk-ui")}'`,
...
async function importPluginModule(url: string): Promise<Record<string, unknown>> {
  ...
  if (!globalThis.__crewspacePluginBridge__) {
 ...
 * self-register via `window.crewspacePlugins.registerReactComponent()`.
 ...
 * of `@crewspaceai/plugin-sdk/ui`, `react`, and `react-dom` resolve to the
