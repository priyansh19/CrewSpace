...
export function OpenClawGatewayConfigFields({
  ...
  return (
    <>
      ...
      {!isCreate && (
        <>
          <Field label="CrewSpace API URL override">
            <DraftInput
              value={
                eff(
                  "adapterConfig",
                  "crewspaceApiUrl",
                  String(config.crewspaceApiUrl ?? ""),
                )
              }
              onCommit={(v) => mark("adapterConfig", "crewspaceApiUrl", v || undefined)}
              immediate
              className={inputClass}
              placeholder="https://crewspace.example"
            />
          </Field>
          ...
          {sessionStrategy === "fixed" && (
            <Field label="Session key">
              <DraftInput
                value={eff("adapterConfig", "sessionKey", String(config.sessionKey ?? "crewspace"))}
                onCommit={(v) => mark("adapterConfig", "sessionKey", v || undefined)}
                immediate
                className={inputClass}
                placeholder="crewspace"
              />
            </Field>
          )}
          ...
          <Field label="Device auth">
            <div className="text-xs text-muted-foreground leading-relaxed">
              Always enabled for gateway agents. CrewSpace persists a device key during onboarding so pairing approvals
