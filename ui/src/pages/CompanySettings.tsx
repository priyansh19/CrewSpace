            ...
            >
              <div className="flex items-center justify-between gap-2">
                ...
                {snippetCopied && (
                  <span
                    ...
                    className="flex items-center gap-1 text-xs text-primary animate-pulse"
                  >
              ...
              <div className="mt-1 space-y-1.5">
                <textarea
                  ...
                  className="h-[28rem] w-full rounded-lg border border-border bg-background px-2 py-1.5 font-mono text-xs outline-none"
                  value={inviteSnippet}
                  readOnly
                />
...
function buildAgentSnippet(input: AgentSnippetInput) {
  ...
  const connectivityBlock =
    candidateUrls.length === 0
      ? `No candidate URLs are available. Ask your user to configure a reachable hostname in CrewSpace, then retry.
Suggested steps:
...
- regenerate this invite snippet`
      : `If none are reachable, ask your user to add a reachable hostname in CrewSpace, restart, and retry.
Suggested command:
- pnpm crewspaceai allowed-hostname <host>
Then verify with: curl -fsS <base-url>/api/health`;

  const resolutionLine = resolutionTestUrl
    ? `\nYou MUST test CrewSpace-to-gateway reachability, call: ${resolutionTestUrl}?url=<urlencoded-gateway-url> (using the hostname that worked above). Do not assume your 172.x is necessarily reachable from CrewSpace. Test it. `
    : "";

  return `You're invited to join a CrewSpace organization.
...
You also need to send a gateway URL that CrewSpace can access. Check your config and include that URL.${resolutionLine}
