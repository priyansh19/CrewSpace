...
/**
 * Filter .crewspace.yaml content so it only includes entries whose
 ...
 * renderYamlBlock.
 */
function filterCrewSpaceYaml(yaml: string, checkedFiles: Set<string>): string {
...
function generateReadmeFromSelection(
  ...
  lines.push("");
  lines.push("This is an [Agent Company](https://crewspace.ing) package.");
  lines.push("");
  ...
  lines.push("");
  lines.push("```bash");
  lines.push("pnpm crewspaceai company import this-github-url-or-folder");
  lines.push("```");
  lines.push("");
  lines.push("See [CrewSpace](https://crewspace.ing) for more information.");
  lines.push("");
  lines.push("---");
  lines.push(`Exported from [CrewSpace](https://crewspace.ing) on ${new Date().toISOString().split("T")[0]}`);
  lines.push("");
...
export function CompanyExport() {
  ...
  // Recompute .crewspace.yaml and README.md content whenever checked files
  ...
  const effectiveFiles = useMemo(() => {
    ...
    // Filter .crewspace.yaml
    const yamlPath = exportData.crewspaceExtensionPath;
    if (yamlPath && typeof exportData.files[yamlPath] === "string") {
      filtered[yamlPath] = filterCrewSpaceYaml(exportData.files[yamlPath], checkedFiles);
    }
