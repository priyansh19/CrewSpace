...
async function ensureEmbeddedPostgresConnection(
  ...
  const preferredAdminConnectionString = `postgres://crewspace:crewspace@127.0.0.1:${preferredPort}/postgres`;
  ...
  if (!runningPid && existsSync(pgVersionFile)) {
    try {
      ...
      await ensurePostgresDatabase(preferredAdminConnectionString, "crewspace");
      process.emitWarning(
      ...
      return {
        connectionString: `postgres://crewspace:crewspace@127.0.0.1:${preferredPort}/crewspace`,
  ...
  if (runningPid) {
    ...
    const adminConnectionString = `postgres://crewspace:crewspace@127.0.0.1:${port}/postgres`;
    await ensurePostgresDatabase(adminConnectionString, "crewspace");
    return {
      connectionString: `postgres://crewspace:crewspace@127.0.0.1:${port}/crewspace`,
  ...
  const adminConnectionString = `postgres://crewspace:crewspace@127.0.0.1:${selectedPort}/postgres`;
  await ensurePostgresDatabase(adminConnectionString, "crewspace");

  return {
    connectionString: `postgres://crewspace:crewspace@127.0.0.1:${selectedPort}/crewspace`,
