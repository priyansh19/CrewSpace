...
export interface CrewSpaceSkillEntry {
  key: string;
  runtimeName: string;
  source: string;
  required?: boolean;
...
interface PersistentSkillSnapshotOptions {
  adapterType: string;
  availableEntries: CrewSpaceSkillEntry[];
...
export function buildCrewSpaceEnv(agent: { id: string; companyId: string }): Record<string, string> {
  ...
  const runtimeHost = resolveHostForUrl(
    process.env.CREWSPACE_LISTEN_HOST ?? process.env.HOST ?? "localhost",
  );
...
export async function resolveCrewSpaceSkillsDir(
  moduleDir: string,
...
export async function listCrewSpaceSkillEntries(
  moduleDir: string,
  additionalCandidates: string[] = [],
...
function normalizeConfiguredCrewSpaceRuntimeSkills(value: unknown): CrewSpaceSkillEntry[] {
...
export async function readCrewSpaceRuntimeSkillEntries(
...
export async function readCrewSpaceSkillMarkdown(
  moduleDir: string,
  skillKey: string,
): Promise<string | null> {
...
export function readCrewSpaceSkillSyncPreference(config: Record<string, unknown>): {
  explicit: boolean;
  desiredSkills: string[];
} {
...
function canonicalizeDesiredCrewSpaceSkillReference(
  reference: string,
...
export function resolveCrewSpaceDesiredSkillNames(
  config: Record<string, unknown>,
...
export function writeCrewSpaceSkillSyncPreference(
  config: Record<string, unknown>,
  desiredSkills: string[],
...
export async function ensureCrewSpaceSkillSymlink(
  source: string,
  target: string,
