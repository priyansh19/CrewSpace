...
async function isLikelyCrewSpaceRepoRoot(candidate: string): Promise<boolean> {
...
async function isLikelyCrewSpaceRuntimeSkillPath(
  candidate: string,
  skillName: string,
  ...
  for (let depth = 0; depth < 6; depth += 1) {
    if (await isLikelyCrewSpaceRepoRoot(cursor)) return true;
...
async function pruneBrokenUnavailableCrewSpaceSkillSymlinks(
  skillsHome: string,
  ...
  for (const entry of entries) {
    ...
    if (
      !(await isLikelyCrewSpaceRuntimeSkillPath(resolvedLinkedPath, entry.name, {
    ...
    await onLog(
      "stdout",
      `[crewspace] Removed stale Codex skill "${entry.name}" from ${skillsHome}\n`,
    );
  }
}
...
export async function ensureCodexSkillsInjected(
  ...
  const allSkillsEntries = options.skillsEntries ?? await readCrewSpaceRuntimeSkillEntries({}, __moduleDir);
    ...
    try {
      ...
      if (existing?.isSymbolicLink()) {
        ...
        if (
          ...
          (await isLikelyCrewSpaceRuntimeSkillPath(resolvedLinkedPath, entry.runtimeName))
        ) {
          ...
          await onLog(
            "stdout",
            `[crewspace] Repaired Codex skill "${entry.runtimeName}" into ${skillsHome}\n`,
          );
          continue;
        }
      }

      const result = await ensureCrewSpaceSkillSymlink(entry.source, target, linkSkill);
      if (result === "skipped") continue;

      await onLog(
        "stdout",
        `[crewspace] ${result === "repaired" ? "Repaired" : "Injected"} Codex skill "${entry.runtimeName}" into ${skillsHome}\n`,
      );
    } catch (err) {
      await onLog(
        "stderr",
        `[crewspace] Failed to inject Codex skill "${entry.key}" into ${skillsHome}: ${err instanceof Error ? err.message : String(err)}\n`,
      );
    }
  }

  await pruneBrokenUnavailableCrewSpaceSkillSymlinks(
    skillsHome,
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  const workspaceHints = Array.isArray(context.crewspaceWorkspaces)
    ...
    : [];
  const runtimeServiceIntents = Array.isArray(context.crewspaceRuntimeServiceIntents)
    ? context.crewspaceRuntimeServiceIntents.filter(
        (value): value is Record<string, unknown> => typeof value === "object" && value !== null,
      )
    : [];
  const runtimeServices = Array.isArray(context.crewspaceRuntimeServices)
    ? context.crewspaceRuntimeServices.filter(
        (value): value is Record<string, unknown> => typeof value === "object" && value !== null,
      )
    : [];
  const runtimePrimaryUrl = asString(context.crewspaceRuntimePrimaryUrl, "");
  ...
  const configuredCodexHome =
    typeof envConfig.CODEX_HOME === "string" && envConfig.CODEX_HOME.trim().length > 0
      ...
      : null;
  const codexSkillEntries = await readCrewSpaceRuntimeSkillEntries(config, __moduleDir);
  ...
  if (effectiveWorkspaceCwd) {
    env.CREWSPACE_WORKSPACE_CWD = effectiveWorkspaceCwd;
  }
  if (workspaceSource) {
    env.CREWSPACE_WORKSPACE_SOURCE = workspaceSource;
  }
  if (workspaceStrategy) {
    env.CREWSPACE_WORKSPACE_STRATEGY = workspaceStrategy;
  }
  if (workspaceId) {
  ...
  if (workspaceBranch) {
    env.CREWSPACE_WORKSPACE_BRANCH = workspaceBranch;
  }
  if (workspaceWorktreePath) {
    env.CREWSPACE_WORKSPACE_WORKTREE_PATH = workspaceWorktreePath;
  }
  if (agentHome) {
  ...
  if (runtimeServiceIntents.length > 0) {
    env.CREWSPACE_RUNTIME_SERVICE_INTENTS_JSON = JSON.stringify(runtimeServiceIntents);
  }
  if (runtimeServices.length > 0) {
    env.CREWSPACE_RUNTIME_SERVICES_JSON = JSON.stringify(runtimeServices);
  }
  if (runtimePrimaryUrl) {
    env.CREWSPACE_RUNTIME_PRIMARY_URL = runtimePrimaryUrl;
  }
  ...
  if (runtimeSessionId && !canResumeSession) {
    await onLog(
      "stdout",
      `[crewspace] Codex session "${runtimeSessionId}" was saved for cwd "${runtimeSessionCwd}" and will not be resumed in "${cwd}".\n`,
    );
  }
  ...
  if (
    ...
    await onLog(
      "stdout",
      `[crewspace] Codex resume session "${sessionId}" is unavailable; retrying with a fresh session.\n`,
    );
