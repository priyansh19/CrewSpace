...
import type { AdapterModel } from "@crewspaceai/adapter-utils";
import { asString, runChildProcess } from "@crewspaceai/adapter-utils/server-utils";
...
function resolvePiCommand(input: unknown): string {
  const envOverride =
    typeof process.env.CREWSPACE_PI_COMMAND === "string" &&
    process.env.CREWSPACE_PI_COMMAND.trim().length > 0
      ? process.env.CREWSPACE_PI_COMMAND.trim()
      : "pi";
  return asString(input, envOverride);
}

const discoveryCache = new Map<string, { expiresAt: number; models: AdapterModel[] }>();
const VOLATILE_ENV_KEY_PREFIXES = ["CREWSPACE_", "npm_", "NPM_"] as const;
