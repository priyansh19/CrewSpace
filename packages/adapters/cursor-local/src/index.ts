...
Use when:
- You want CrewSpace to run Cursor Agent CLI locally as the agent runtime
...
- CrewSpace auto-injects local skills into "~/.cursor/skills" when missing, so Cursor can discover "$crewspace" and related skills on local runs.
- CrewSpace auto-adds --yolo unless one of --trust/--yolo/-f is already present in extraArgs.
`;
