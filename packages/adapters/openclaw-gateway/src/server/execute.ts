...
import { asNumber, asString, buildCrewSpaceEnv, parseObject } from "@crewspaceai/adapter-utils/server-utils";
...
const DEFAULT_CLIENT_VERSION = "crewspace";
...
function resolveSessionKey(input: {
  ...
  const fallback = input.configuredSessionKey ?? "crewspace";
  if (input.strategy === "run") return `crewspace:run:${input.runId}`;
  if (input.strategy === "issue" && input.issueId) return `crewspace:issue:${input.issueId}`;
  return fallback;
}
...
function resolveCrewSpaceApiUrlOverride(value: unknown): string | null {
...
function buildCrewSpaceEnvForWake(ctx: AdapterExecutionContext, wakePayload: WakePayload): Record<string, string> {
  const crewspaceApiUrlOverride = resolveCrewSpaceApiUrlOverride(ctx.config.crewspaceApiUrl);
  const crewspaceEnv: Record<string, string> = {
    ...buildCrewSpaceEnv(ctx.agent),
    CREWSPACE_RUN_ID: ctx.runId,
  };

  if (crewspaceApiUrlOverride) {
    crewspaceEnv.CREWSPACE_API_URL = crewspaceApiUrlOverride;
  }
  if (wakePayload.taskId) crewspaceEnv.CREWSPACE_TASK_ID = wakePayload.taskId;
  if (wakePayload.wakeReason) crewspaceEnv.CREWSPACE_WAKE_REASON = wakePayload.wakeReason;
  if (wakePayload.wakeCommentId) crewspaceEnv.CREWSPACE_WAKE_COMMENT_ID = wakePayload.wakeCommentId;
  if (wakePayload.approvalId) crewspaceEnv.CREWSPACE_APPROVAL_ID = wakePayload.approvalId;
  if (wakePayload.approvalStatus) crewspaceEnv.CREWSPACE_APPROVAL_STATUS = wakePayload.approvalStatus;
  if (wakePayload.issueIds.length > 0) {
    crewspaceEnv.CREWSPACE_LINKED_ISSUE_IDS = wakePayload.issueIds.join(",");
  }

  return crewspaceEnv;
}

function buildWakeText(payload: WakePayload, crewspaceEnv: Record<string, string>): string {
  const claimedApiKeyPath = "~/.openclaw/workspace/crewspace-claimed-api-key.json";
  const orderedKeys = [
    "CREWSPACE_RUN_ID",
    "CREWSPACE_AGENT_ID",
    "CREWSPACE_COMPANY_ID",
    "CREWSPACE_API_URL",
    "CREWSPACE_TASK_ID",
    "CREWSPACE_WAKE_REASON",
    "CREWSPACE_WAKE_COMMENT_ID",
  ...
  for (const key of orderedKeys) {
    const value = crewspaceEnv[key];
    if (!value) continue;
    envLines.push(`${key}=${value}`);
  }

  const issueIdHint = payload.taskId ?? payload.issueId ?? "";
  const apiBaseHint = crewspaceEnv.CREWSPACE_API_URL ?? "<set CREWSPACE_API_URL>";

  const lines = [
    "CrewSpace wake event for a cloud adapter.",
    "",
    "Run this procedure now. Do not guess undocumented endpoints and do not ask for additional heartbeat docs.",
    "",
    "Set these values in your run context:",
    ...envLines,
    `CREWSPACE_API_KEY=<token from ${claimedApiKeyPath}>`,
    "",
    `Load CREWSPACE_API_KEY from ${claimedApiKeyPath} (the token you saved after claim-api-key).`,
    "",
    ...
    "",
    "HTTP rules:",
    "- Use Authorization: Bearer $CREWSPACE_API_KEY on every API call.",
    "- Use X-CrewSpace-Run-Id: $CREWSPACE_RUN_ID on every mutating API call.",
    "- Use only /api endpoints listed below.",
    "- Do NOT call guessed endpoints like /api/cloud-adapter/*, /api/cloud-adapters/*, /api/adapters/cloud/*, or /api/heartbeat.",
    "",
    "Workflow:",
    "1) GET /api/agents/me",
    `2) Determine issueId: CREWSPACE_TASK_ID if present, otherwise issue_id (${issueIdHint}).`,
    "3) If issueId exists:",
    "   - POST /api/issues/{issueId}/checkout with {\"agentId\":\"$CREWSPACE_AGENT_ID\",\"expectedStatuses\":[\"todo\",\"backlog\",\"blocked\"]}",
    ...
    "   - GET /api/companies/$CREWSPACE_COMPANY_ID/issues?assigneeAgentId=$CREWSPACE_AGENT_ID&status=todo,in_progress,blocked",
...
function buildStandardCrewSpacePayload(
  ctx: AdapterExecutionContext,
  wakePayload: WakePayload,
  crewspaceEnv: Record<string, string>,
  payloadTemplate: Record<string, unknown>,
): Record<string, unknown> {
  const templateCrewSpace = parseObject(payloadTemplate.crewspace);
  const workspace = asRecord(ctx.context.crewspaceWorkspace);
  const workspaces = Array.isArray(ctx.context.crewspaceWorkspaces)
    ? ctx.context.crewspaceWorkspaces.filter((entry): entry is Record<string, unknown> => Boolean(asRecord(entry)))
    : [];
  const configuredWorkspaceRuntime = parseObject(ctx.config.workspaceRuntime);
  const runtimeServiceIntents = Array.isArray(ctx.context.crewspaceRuntimeServiceIntents)
    ? ctx.context.crewspaceRuntimeServiceIntents.filter(
        (entry): entry is Record<string, unknown> => Boolean(asRecord(entry)),
      )
    : [];

  const standardCrewSpace: Record<string, unknown> = {
    runId: ctx.runId,
    ...
    apiUrl: crewspaceEnv.CREWSPACE_API_URL ?? null,
  };

  if (workspace) {
    standardCrewSpace.workspace = workspace;
  }
  if (workspaces.length > 0) {
    standardCrewSpace.workspaces = workspaces;
  }
  if (runtimeServiceIntents.length > 0 || Object.keys(configuredWorkspaceRuntime).length > 0) {
    standardCrewSpace.workspaceRuntime = {
...
class GatewayWsClient {
  ...
  close() {
    if (!this.ws) return;
    this.ws.close(1000, "crewspace-complete");
    this.ws = null;
  }
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  const crewspaceEnv = buildCrewSpaceEnvForWake(ctx, wakePayload);
  const wakeText = buildWakeText(wakePayload, crewspaceEnv);
  ...
  const crewspacePayload = buildStandardCrewSpacePayload(ctx, wakePayload, crewspaceEnv, payloadTemplate);
