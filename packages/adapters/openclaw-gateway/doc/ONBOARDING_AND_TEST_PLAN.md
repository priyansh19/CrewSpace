...
## Scope
This plan is now **gateway-only**. CrewSpace supports OpenClaw through `openclaw_gateway` only.
...
6. OpenClaw claims API key and installs/uses CrewSpace skill.
...
3. In `/new` OpenClaw session, OpenClaw can still create a CrewSpace task.
...
```bash
pnpm --filter @crewspace/server test:run -- openclaw-gateway
pnpm --filter @crewspace/server typecheck
pnpm --filter @crewspace/ui typecheck
pnpm --filter crewspace typecheck
```
