...
This document describes how `@crewspace/adapter-openclaw-gateway` invokes OpenClaw over the Gateway protocol.
...
5. stream `event agent` frames into CrewSpace logs/transcript parsing
...
- required fields:
  ...
  - `idempotencyKey` (CrewSpace `runId`)
