...
function packLocalPackage(packagePath: string, outputDir: string): string {
  ...
  const sdkBundleDir = path.join(outputDir, ".crewspace-sdk");
...
/**
 * Generate a complete CrewSpace plugin starter project.
 *
...
export function scaffoldPluginProject(options: ScaffoldPluginOptions): string {
  ...
  const description = options.description ?? "A CrewSpace plugin";
  ...
  const packageJson = {
    ...
    scripts: {
      ...
      "dev:ui": "crewspace-plugin-dev-server --root . --ui-dir dist/ui --port 4177",
    ...
    keywords: ["crewspace", "plugin", category],
    author,
    license: "MIT",
    ...(packedSharedTarball
      ? {
        pnpm: {
          overrides: {
            "@crewspaceai/shared": `file:${toPosixPath(path.relative(outputDir, packedSharedTarball))}`,
          },
        },
      }
      : {}),
    devDependencies: {
      ...(packedSharedTarball
        ...
        : {}),
      "@crewspaceai/plugin-sdk": sdkDependency,
...
import { createPluginBundlerPresets } from "@crewspaceai/plugin-sdk/bundlers";
...
`,
  ...
  writeFile(
    ...
    `import { definePlugin, runWorker } from "@crewspaceai/plugin-sdk";
...
`,
  ...
  writeFile(
    ...
    `import { usePluginAction, usePluginData, type PluginWidgetProps } from "@crewspaceai/plugin-sdk/ui";
...
import { createTestHarness } from "@crewspaceai/plugin-sdk/testing";
...
${sdkDependency.startsWith("file:")
  ? `This scaffold snapshots \`@crewspaceai/plugin-sdk\` and \`@crewspaceai/shared\` from a local CrewSpace checkout at:\n\n\`${toPosixPath(localSdkPath)}\`\n\nThe packed tarballs live in \`.crewspace-sdk/\` for local development. Before publishing this plugin, switch those dependencies to published package versions once they are available on npm.\n\n`
  : ""}
...
- \`pnpm build\` uses esbuild presets from \`@crewspaceai/plugin-sdk/bundlers\`.
...
function runCli() {
  ...
  if (!pluginName) {
    ...
    console.error("Usage: create-crewspace-plugin <name> [--template default|connector|workspace] [--output <dir>] [--sdk-path <crewspace-sdk-path>]");
    process.exit(1);
  }
