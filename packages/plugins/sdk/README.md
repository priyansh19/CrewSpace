# `@crewspace/plugin-sdk`

Official TypeScript SDK for CrewSpace plugin authors.

- **Worker SDK:** `@crewspace/plugin-sdk` — `definePlugin`, context, lifecycle
- **UI SDK:** `@crewspace/plugin-sdk/ui` — React hooks and slot props
- **Testing:** `@crewspace/plugin-sdk/testing` — in-memory host harness
- **Bundlers:** `@crewspace/plugin-sdk/bundlers` — esbuild/rollup presets
- **Dev server:** `@crewspace/plugin-sdk/dev-server` — static UI server + SSE reload
...
| Import | Purpose |
|--------|--------|
| `@crewspace/plugin-sdk` | Worker entry: `definePlugin`, `runWorker`, context types, protocol helpers |
| `@crewspace/plugin-sdk/ui` | UI entry: `usePluginData`, `usePluginAction`, `usePluginStream`, `useHostContext`, slot prop types |
| `@crewspace/plugin-sdk/ui/hooks` | Hooks only |
| `@crewspace/plugin-sdk/ui/types` | UI types and slot prop interfaces |
| `@crewspace/plugin-sdk/testing` | `createTestHarness` for unit/integration tests |
| `@crewspace/plugin-sdk/bundlers` | `createPluginBundlerPresets` for worker/manifest/ui builds |
| `@crewspace/plugin-sdk/dev-server` | `startPluginDevServer`, `getUiBuildSnapshot` |
| `@crewspace/plugin-sdk/protocol` | JSON-RPC protocol types and helpers (advanced) |
| `@crewspace/plugin-sdk/types` | Worker context and API types (advanced) |
...
- Plugin UI bundles run as same-origin JavaScript inside the main CrewSpace app. They can call ordinary CrewSpace HTTP APIs with the board session, so manifest capabilities are not a frontend sandbox.
...
- For deployed plugins, publish an npm package and install that package into the CrewSpace instance at runtime.
...
```ts
import { definePlugin, runWorker } from "@crewspace/plugin-sdk";
...
**Entity types** (for `entityTypes` on slots): `project` \| `issue` \| `agent` \| `goal` \| `run` \| `comment`. Full list: import `PLUGIN_UI_SLOT_TYPES` and `PLUGIN_UI_SLOT_ENTITY_TYPES` from `@crewspace/plugin-sdk`.
...
A card or section rendered on the main dashboard. Use this for at-a-glance metrics, status indicators, or summary views that surface plugin data alongside core CrewSpace information. Receives `PluginWidgetProps` with `context.companyId` set to the active company. Requires the `ui.dashboardWidget.register` capability.
...
Full list in code: import `PLUGIN_CAPABILITIES` from `@crewspace/plugin-sdk`.

## UI quick start

```tsx
import { usePluginData, usePluginAction } from "@crewspace/plugin-sdk/ui";
...
```tsx
import { usePluginData } from "@crewspace/plugin-sdk/ui";
...
import { usePluginAction, type PluginBridgeError } from "@crewspace/plugin-sdk/ui";
...
```tsx
import { useHostContext, usePluginData } from "@crewspace/plugin-sdk/ui";
import type { PluginDetailTabProps } from "@crewspace/plugin-sdk/ui";
...
```tsx
import { usePluginStream } from "@crewspace/plugin-sdk/ui";
...
Each slot type receives a typed props object with `context: PluginHostContext`. Import from `@crewspace/plugin-sdk/ui`.
...
```tsx
import type { PluginProjectSidebarItemProps } from "@crewspace/plugin-sdk/ui";
...
```tsx
import { useState } from "react";
import { usePluginAction, usePluginStream } from "@crewspace/plugin-sdk/ui";
...
```ts
import { createTestHarness } from "@crewspace/plugin-sdk/testing";
...
```ts
import { createPluginBundlerPresets } from "@crewspace/plugin-sdk/bundlers";
...
```bash
crewspace-plugin-dev-server --root . --ui-dir dist/ui --port 4177
```

Or programmatically:

```ts
import { startPluginDevServer } from "@crewspace/plugin-sdk/dev-server";
const server = await startPluginDevServer({ rootDir: process.cwd() });
```

Dev server endpoints:
- `GET /__crewspace__/health` returns `{ ok, rootDir, uiDir }`
- `GET /__crewspace__/events` streams `reload` SSE events on UI build changes
