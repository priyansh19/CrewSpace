/**
 * Core types for the CrewSpace plugin worker-side SDK.
 *
 * These types define the stable public API surface that plugin workers import
 * from `@crewspaceai/plugin-sdk`.  The host provides a concrete implementation
...
// ---------------------------------------------------------------------------
// Re-exports from @crewspaceai/shared (plugin authors import from one place)
// ---------------------------------------------------------------------------
...
export interface ScopeKey {
  /** What kind of CrewSpace object this state is scoped to. */
...
export interface PluginEntityUpsert {
  ...
  externalId?: string;
  /** Human-readable title for display in the CrewSpace UI. */
  title?: string;
...
/**
 * `ctx.events` — subscribe to and emit CrewSpace domain events.
 *
...
export interface PluginEventsClient {
  /**
   * Subscribe to a core CrewSpace domain event or a plugin-namespaced event.
   *
 ...
 * This client resolves the reference through the CrewSpace secret provider
...
export interface PluginSecretsClient {
   ...
   * in the CrewSpace secret provider (e.g. `"MY_API_KEY"`).
   *
 ...
 * ```
 *
 * @see usePluginStream in `@crewspaceai/plugin-sdk/ui`
 */
