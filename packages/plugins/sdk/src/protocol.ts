 ...
 * channel.
 *
 * The CrewSpace plugin runtime uses JSON-RPC 2.0 over stdio to communicate
...
export type { PluginLauncherRenderContextSnapshot } from "@crewspaceai/shared";
 ...
 * we use strings (UUIDs or monotonic counters) for all CrewSpace messages.
 */
...
/**
 * CrewSpace plugin-specific error codes.
 *
...
export interface InitializeParams {
  ...
  instanceInfo: {
    /** UUID of this CrewSpace instance. */
    instanceId: string;
    /** Semver version of the running CrewSpace host. */
    hostVersion: string;
  };
  /** Host API version. */
  apiVersion: number;
