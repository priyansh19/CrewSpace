...
Example CrewSpace plugin that demonstrates:
...
```bash
pnpm --filter @crewspace/plugin-file-browser-example build
pnpm crewspace plugin install ./packages/plugins/examples/plugin-file-browser-example
```

To uninstall:

```bash
pnpm crewspace plugin uninstall crewspace-file-browser-example --force
```
...
- Optional: use `crewspace-plugin-dev-server` for UI hot-reload with `devUiUrl` in plugin config.
