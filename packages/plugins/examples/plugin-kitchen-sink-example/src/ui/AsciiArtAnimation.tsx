...
type CrewSpaceSprite = (typeof CREWSPACE_SPRITES)[number];
...
function spriteSize(sprite: CrewSpaceSprite): { width: number; height: number } {
  let width = 0;
...
export function AsciiArtAnimation() {
  ...
  useEffect(() => {
    ...
    function drawStaticFrame() {
      ...
      for (let baseRow = 1; baseRow < rows - 9; baseRow += gapY) {
        ...
        for (let baseCol = startX; baseCol < cols - 10; baseCol += gapX) {
          const sprite = CREWSPACE_SPRITES[(baseCol + baseRow) % CREWSPACE_SPRITES.length]!;
    ...
    function spawnClip() {
      const sprite = CREWSPACE_SPRITES[Math.floor(Math.random() * CREWSPACE_SPRITES.length)]!;
